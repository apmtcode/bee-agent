...
## 2026-07-10 (run 9) — 🐛 Fix background-task state corruption + de-flake the suite

**Audited:** The actual test suite health (not just typecheck). `npm test` was
**red**: 3 tests failed *deterministically* with
`SyntaxError: Expected ',' or '}' after property value in JSON` thrown from
`readJsonFile` while reconciling background-task state. Prior run logs claimed
174/174 — the environment now surfaces the race every time. Traced it to a real
bug, not a test bug.

**Root cause (two layers):**
1. **Malformed JSON, not just a race.** The launch script generated by
   `renderLaunchScript` (in `src/harness/background-tasks.ts` *and*
   `src/training/runner.ts`) built the initial `running` state by
   `printf '%s' '<json>' | sed "…; s/\"\$\$\"/$$/g" > state.json`. In `sed`
   BRE `$` is an end-of-line anchor, so `"$$"` never matched — and worse, any
   task **command containing quotes/`$`** (e.g. `printf 'l%s\n' "$i"`) produced
   an unescaped `"` in the JSON, so the state file was *consistently* invalid.
   The Python completion writer then did `json.loads(state_path.read_text())` on
   that garbage and errored, so the task never reached a terminal state either.
2. **Non-atomic writes.** Even with valid content, the shell `>` redirect and
   Python `write_text` truncate-in-place, so a concurrent `reconcileTask` reader
   could catch a partial file.

**Changed (source, additive):**
- Replaced the fragile `printf|sed` JSON assembly with a Python
  `renderRunningStateWriterPython` helper that receives every dynamic value via
  **argv** (never re-parsed as JSON syntax) and writes **atomically**
  (temp sibling → `os.replace`). Applied in both `background-tasks.ts` and
  `training/runner.ts`.
- Made the terminal-state Python writers atomic too (temp + `os.replace`).

**Changed (tests):**
- New regression test in `background-tasks.test.ts`: actually executes a
  generated launch script and hammers `readState` in a tight loop, asserting
  **zero** parse failures and a parseable terminal state. Fails hard on the old
  code, passes on the new.
- Updated `runner.test.ts` to assert the new atomic writer (`python3 - '<state>'`
  + `os.replace(tmp_path, state_path)`) instead of the removed `> '<state>'`.
- **De-flaked** three suites that spawned *real* subprocesses while manually
  driving state via `writeState`/`writeOutput`: the real launch script wrote its
  own `running` state asynchronously and, under parallel load, raced the manual
  writes (flaked breaker `failureCount 2→3`, `status completed→running`,
  recover arrays). Injected a no-op `backgroundTaskSpawnProcess` in the affected
  runtimes (`server.test.ts` main + drifting + breaker,
  `operator-runtime.test.ts` background-task test) so manual state is the single
  source of truth. These tests already simulate execution manually, so intent is
  preserved.

**Test results:** `npm test` **175/175**, green **10/10** consecutive full-suite
runs (was ~40% flaky before). `typecheck:src` ✅ (source stays clean). Build ✅.

**New idea:** add a tiny `writeJsonAtomic`-style guarantee to the *reader* path
too — a `readJsonFileResilient` that, on a `SyntaxError`, retries a bounded few
times with a micro-delay before surfacing. Atomic writes make this unnecessary
today, but it's cheap defense-in-depth for any future non-atomic writer (e.g. a
plugin) and would convert a hard crash into at-worst-stale reads. Bigger idea:
a **"no real subprocess in unit tests" lint** — flag any `new
StandaloneOperatorRuntime` in a `*.test.ts` that starts a background task
without injecting `backgroundTaskSpawnProcess`, so this class of flake can't be
reintroduced.
