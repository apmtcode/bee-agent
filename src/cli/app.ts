...
export type OperatorCliAppOptions = {
  ...
  /**
   * Test seam: override how background-task processes are spawned / probed for
   * liveness. Defaults to the real `spawn` / `process.kill(pid, 0)` in
   * production. Tests inject deterministic stand-ins so no real detached
   * process is launched (which otherwise races the test's own state writes).
   */
