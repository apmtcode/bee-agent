...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background-task launch scripts are spawned. Production spawns a
   * real detached process; tests should inject a deterministic stub so their
   * assertions don't race the asynchronous launch script writing state/output.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Override background-task liveness detection (defaults to a real signal-0 probe). */
