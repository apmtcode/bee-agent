...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background-task launch scripts are spawned. Production defaults
   * to a real detached process; tests inject a deterministic no-op spawn so the
   * launch script never actually runs (keeping state-file writes hermetic).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Override the liveness check used when reconciling background tasks. */
