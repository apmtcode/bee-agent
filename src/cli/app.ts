...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional override for how background tasks are spawned. Production leaves
   * this undefined (a real detached process launches). Tests that drive task
   * state explicitly should inject a no-op spawn so a real launch does not race
   * their manual state writes.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Optional override for the "is this pid alive" check (see above). */
