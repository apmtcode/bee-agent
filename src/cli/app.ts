...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional overrides for background-task process management. Tests inject a
   * no-op spawn so unit assertions don't depend on real subprocess timing
   * (a spawned launch script writes state/output asynchronously, racing the
   * test's own writes).
   */
