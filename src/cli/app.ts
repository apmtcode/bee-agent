...
export type OperatorCliAppOptions = {
  ...
  /**
   * Overrides how background-task processes are spawned / probed for liveness.
   * Production leaves these unset (real `spawn` / `process.kill(pid, 0)`);
   * tests inject deterministic stubs so a real detached process cannot race
   * their assertions.
   */
