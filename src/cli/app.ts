...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background tasks are spawned / probed. Production uses the
   * runtime defaults (real detached process + `process.kill` liveness check);
   * tests inject deterministic stubs so no live process races against explicit
   * state writes. Forwarded verbatim to {@link StandaloneOperatorRuntime}.
   */
