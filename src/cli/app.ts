...
export type OperatorCliAppOptions = {
  ...
  /**
   * Overrides for the background-task process seam. Production leaves these
   * undefined (the runtime spawns real detached shells). Tests inject a no-op
   * spawn so background tasks never launch real subprocesses whose asynchronous
   * state writes would race the assertions.
   */
