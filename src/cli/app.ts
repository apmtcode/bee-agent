...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional background-task spawn/liveness overrides forwarded to the runtime.
   * Production leaves these unset (real `spawn` + PID probing); tests inject
   * stubs so no real OS process launches and its asynchronous state writes
   * cannot race with explicit test-driven state transitions.
   */
