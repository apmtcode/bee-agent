...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injectable background-task process seams, forwarded to the runtime. In
   * production these default to a real detached spawn + liveness probe. Tests
   * should supply deterministic stubs so controlled `writeState` calls are not
   * raced by a real subprocess writing its own execution state.
   */
