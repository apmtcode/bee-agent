...
export type OperatorCliAppOptions = {
  ...
  /**
   * Test seam: override how background-task worker processes are spawned and how
   * their liveness is probed. Defaults to the real `spawn`/`process.kill(pid,0)`
   * implementations. Injecting an inert spawn keeps background-task tests
   * hermetic and deterministic (no real child process racing the assertions).
   */
