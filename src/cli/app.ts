...
export type OperatorCliAppOptions = {
  ...
  /**
   * Overrides for the background-task execution backend, forwarded to the
   * runtime. Tests inject a deterministic spawn/liveness pair so background
   * task state is driven entirely by explicit `writeState` calls rather than a
   * real detached process racing the assertions.
   */
