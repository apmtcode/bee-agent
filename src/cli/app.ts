...
export type OperatorCliAppOptions = {
  ...
  /**
   * Test seam: inject a deterministic background-task process spawner so tests
   * never launch a real OS process. Real detached processes write their state
   * file non-atomically (`sed > file`, `python write_text`) and race
   * reconcile's `readState`, which makes any test that starts a background task
   * timing-dependent (torn-read JSON errors, spurious `missing-process`
   * degradation). Production leaves these undefined to use the real `spawn` and
