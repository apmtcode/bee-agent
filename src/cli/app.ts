...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional injection seams for the background-task subsystem, forwarded to the
   * underlying runtime. Production leaves these undefined (real `spawn` / OS
   * process probing); tests pass deterministic fakes so the suite never depends
   * on real process-spawn timing or on transient on-disk execution state.
   */
