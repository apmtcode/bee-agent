...
export type OperatorCliAppOptions = {
  ...
  configHome?: string;
  /**
   * Injectable spawn for background tasks. Production uses the real
   * `child_process.spawn`; tests pass a deterministic stub so the CLI never
   * launches real detached processes (which would race the test's controlled
   * task-state writes).
   */
