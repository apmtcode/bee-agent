...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injectable background-task launcher / liveness probe. Production uses the
   * real `spawn`; tests should stub these so no detached `run.sh` subprocess is
   * spawned (which would write execution-state files asynchronously and race
   * the test's own deterministic transitions).
   */
