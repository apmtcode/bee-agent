...
export type OperatorCliAppOptions = {
  ...
  /**
   * Test seams for the background-task subsystem. Injecting a stub spawn and a
   * deterministic liveness check lets tests exercise background-task flows
   * without spawning real OS processes (which are non-deterministic under load,
   * especially in cloud/CI). Production leaves both undefined.
   */
