...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional background-task executor seams, forwarded to the runtime. Tests
   * inject an inert spawn (and deterministic liveness) so manual state writes
   * are authoritative instead of racing a real detached subprocess. Production
   * leaves these undefined to use the real spawn/liveness implementations.
   */
