...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injection seam for the background-task launcher. Defaults to the real
   * detached-process spawner in production; tests pass a no-op spawn so the
   * suite never launches real OS processes (which would race with explicit
   * state fixtures and make assertions non-deterministic).
   */
