...
export type OperatorCliAppOptions = {
  ...
  /**
   * Inject the background-task process spawner. Defaults to a real detached
   * spawn in production. Tests should pass a deterministic mock so they do not
   * depend on real OS process lifecycle (which is non-hermetic).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Inject the background-task liveness check (defaults to a real `process.kill` probe). */
