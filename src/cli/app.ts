...
export type OperatorCliAppOptions = {
  ...
  configHome?: string;
  /**
   * Optional override for how background tasks spawn OS processes. Defaults to a
   * real `spawn` in production. Tests should inject a no-op that returns a pid
   * without launching a process, so the runtime stays hermetic and the launch
   * script never writes state asynchronously.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /**
   * Optional override for the liveness check used when reconciling background
   * tasks. Defaults to a real `process.kill(pid, 0)` probe. Paired with
   * {@link backgroundTaskSpawnProcess} so tests can keep a task "running"
