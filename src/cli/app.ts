...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injection seams for the background-task execution service. Production leaves
   * these undefined (real `spawn` / process-liveness probe). Tests supply
   * deterministic fakes so background-task behaviour is driven by explicit
   * `writeState()` calls rather than by racing a real detached child.
   */
