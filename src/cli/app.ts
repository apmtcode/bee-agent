...
export type OperatorCliAppOptions = {
  ...
  /**
   * Background-task execution overrides forwarded to the runtime. In
   * production these are left unset (real detached processes); tests and
   * simulations inject a deterministic driver / spawn so background execution
  ...
  backgroundExecutionDriver?: StandaloneOperatorOptions["backgroundExecutionDriver"];
};
...
export class OperatorCliApp {
  ...
  constructor(private readonly options: OperatorCliAppOptions) {
    this.runtime = new StandaloneOperatorRuntime({
      ...
      backgroundExecutionDriver: options.backgroundExecutionDriver,
    });
