...
export type OperatorCliAppOptions = {
  ...
  /**
   * Test seam: override how background tasks spawn / are probed for liveness.
   * Production leaves these undefined (real `spawn` + PID probe). Tests inject
   * a no-op spawn so background tasks don't launch real detached processes that
   * race to write their state files against the test's own expectations.
   */
