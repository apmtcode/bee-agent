...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override the background-task OS seams (process spawn + liveness probe).
   * Production uses the real detached-child launcher; tests and simulations
   * inject a deterministic implementation (see
   * `createSimulatedBackgroundExecution`) so no real OS process is spawned.
   */
