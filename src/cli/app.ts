...
export type OperatorCliAppOptions = {
  ...
  /**
   * Seam for background-task process spawning. Defaults to the real `spawn`
   * in production. Tests should inject a deterministic fake so no real OS
   * subprocesses are launched (which would race with the test and degrade
   * platform-control state non-deterministically in CI).
   */
