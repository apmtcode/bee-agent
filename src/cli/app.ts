...
export type OperatorCliAppOptions = {
  ...
  /**
   * Pluggable background-task process backend. Threaded into the internal
   * runtime's {@link FileBackgroundTaskStore}. Production leaves these undefined
   * (real `spawn` + `process.kill` liveness probe); embedders and tests can
   * inject a mock so no real OS process is launched.
   */
