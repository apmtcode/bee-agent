...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional overrides forwarded to the underlying runtime's background-task
   * store. Production leaves these unset (real `spawn` + liveness probe); tests
   * inject deterministic stubs so background-task assertions don't race a real
   * detached process writing the same state file.
   */
