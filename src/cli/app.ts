...
export type OperatorCliAppOptions = {
  ...
  /**
   * Seam for injecting a fake background-task process launcher. Defaults to the
   * real `spawn`-backed launcher in production; tests pass a no-op so background
   * tasks never fork a real OS process (which would race the test's own state
   * writes and is not available in every sandbox).
   */
