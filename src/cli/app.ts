...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background tasks spawn their detached launch process. In
   * production this defaults to the runtime's real `spawn`; tests inject an
   * inert stub so no real OS process runs and background-task state stays fully
   * deterministic (no races between the launch script and the runtime's reads).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Override the liveness probe for background-task processes (see above). */
