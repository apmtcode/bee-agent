...
export type OperatorCliAppOptions = {
  ...
  /**
   * How background-task processes are spawned. Defaults to a real
   * `child_process.spawn` in production. Tests should inject a no-op spawn so
   * the suite stays hermetic — no real OS processes, no state-file races.
   */
   ...
   * `process.kill(pid, 0)` check. Tests should inject a deterministic probe.
   */
