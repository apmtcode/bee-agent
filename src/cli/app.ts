...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background tasks spawn their launch process. Production spawns
   * a real detached shell; tests inject a deterministic stub so the suite never
   * depends on real subprocess timing (which is inherently non-hermetic).
   */
  backgroundTaskSpawnProcess?: ConstructorParameters<typeof StandaloneOperatorRuntime>[0]["backgroundTaskSpawnProcess"];
  /** Override the liveness probe used when reconciling background tasks (tests). */
