...
export type OperatorCliAppOptions = {
   ...
   * `child_process.spawn`. Tests and hermetic (cloud/CI) environments should
   * pass {@link createInertBackgroundSpawn} so background-task commands do not
   * launch real detached OS processes.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Injectable liveness probe for background-task pids (see above). */
