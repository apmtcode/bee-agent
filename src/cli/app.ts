...
export type OperatorCliAppOptions = {
  ...
  /**
   * Overrides for how background tasks are launched and probed. Production
   * leaves these undefined (the runtime spawns a real detached process and
   * probes liveness via `process.kill(pid, 0)`). Tests inject a non-spawning
   * stub so the launcher's asynchronous state writes cannot race assertions.
   */
