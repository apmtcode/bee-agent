...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injectable background-task process spawner, forwarded to the runtime. Tests
   * pass a no-op spawner so background tasks never launch real OS processes (whose
   * async state writes would otherwise race with assertions). Defaults to the
   * runtime's real `spawn` in production.
   */
