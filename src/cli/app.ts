...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injectable background-task process control, forwarded to the underlying
   * {@link StandaloneOperatorRuntime}. Production leaves these undefined (real
   * `spawn`/`process.kill`); tests inject deterministic stubs so background
   * task reconciliation never depends on real OS process timing.
   */
