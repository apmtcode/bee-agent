...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background tasks are spawned / whether their process is
   * considered alive. Production leaves these undefined (real `spawn` +
   * `process.kill(pid, 0)`); tests inject deterministic stand-ins so the suite
   * never depends on real OS process timing.
   */
