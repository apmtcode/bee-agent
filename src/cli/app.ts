...
export type OperatorCliAppOptions = {
  ...
  /**
   * Test seam: override how background-task launch scripts are spawned. Tests
   * should pass a no-op spawn so no real detached subprocess writes its own
   * `state.json` asynchronously and races with the test's assertions.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Test seam: override the liveness check used for background-task PIDs. */
