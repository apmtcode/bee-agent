...
export type OperatorCliAppOptions = {
  ...
  configHome?: string;
  /**
   * Injectable spawn for background tasks. Defaults to a real `child_process`
   * spawn in production. Tests should pass a deterministic mock so the app
   * never launches real detached OS processes (which race the recorded state
   * file and make the suite environment-dependent).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Injectable process-liveness probe for background tasks (see above). */
