...
export type OperatorCliAppOptions = {
  ...
  /**
   * Overrides for background-task process control. Production uses the runtime
   * defaults (real `child_process` spawn + `process.kill(pid, 0)` liveness).
   * Tests inject deterministic stand-ins so recovery/failure state does not
   * depend on real subprocess timing.
   */
