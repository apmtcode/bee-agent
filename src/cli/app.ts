...
export type OperatorCliAppOptions = {
  ...
  /**
   * Test seam: override how background-task launcher processes are spawned and
   * how process liveness is probed. Forwarded to the runtime so tests can drive
   * background-task state deterministically instead of racing a real launch
   * script. Left undefined in production (real `spawn` / `process.kill` probe).
   */
