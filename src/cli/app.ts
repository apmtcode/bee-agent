...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional background-task hooks forwarded to the underlying runtime.
   * Production leaves these undefined (real `spawn` / liveness probe); tests
   * inject deterministic stubs so background tasks never launch real detached
   * processes that would otherwise race with assertions.
   */
