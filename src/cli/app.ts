...
export type OperatorCliAppOptions = {
  ...
  /**
   * Overrides for the background-task execution seam. Production leaves these
   * undefined (real `child_process.spawn` + `process.kill` liveness probe).
   * Tests inject a no-op spawn so a real detached subprocess never races the
   * state files they write by hand, keeping background-task assertions
   * deterministic.
   */
