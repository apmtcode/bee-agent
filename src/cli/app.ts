...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional spawn backend for background tasks. Defaults to the runtime's real
   * OS spawn. Supply {@link createSimulatedBackgroundSpawn} to run background-task
   * orchestration without launching real processes (cloud, CI, hermetic tests).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Optional liveness probe for background-task pids (defaults to the real check). */
