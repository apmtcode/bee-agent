...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional injection seam for how background tasks spawn their launch
   * process. Production uses the default `spawn`; tests pass a side-effect-free
   * stub so no real subprocess (and no racing state-file write) is created.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Optional injection seam for the liveness probe used during task recovery. */
