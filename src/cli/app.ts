...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injectable background-task process spawner. Defaults to the real
   * `child_process.spawn`. Tests should pass a no-op stub (e.g.
   * `() => ({ pid: 4242, unref() {} })`) so background-task assertions never
   * depend on real OS subprocess timing.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /**
   * Injectable liveness probe for background-task pids. Defaults to a real
   * `process.kill(pid, 0)` check. Tests should pass a deterministic predicate.
   */
