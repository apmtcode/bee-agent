...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injection seams for the background-task subsystem, forwarded to the
   * runtime. Production leaves these unset (real `spawn` + live process
   * checks); tests pass deterministic mocks so background tasks never spawn
   * real subprocesses that would race the test's own state manipulation.
   */
