...
export type OperatorCliAppOptions = {
  ...
  /**
   * How detached background-task processes are spawned. Defaults to a real
   * `child_process.spawn`. Tests inject a deterministic fake so they never
   * depend on real OS process timing (which otherwise races the state file).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /**
   * How a background-task PID's liveness is probed. Defaults to a real
   * `process.kill(pid, 0)` check. Tests inject a deterministic fake.
   */
