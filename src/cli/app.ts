...
export type OperatorCliAppOptions = {
  ...
  /**
   * Test seams for the background-task subsystem. In production the runtime
   * spawns a real detached shell; tests inject a deterministic no-op spawn (and
   * optionally an `isProcessRunning` probe) so background-task behaviour does not
   * depend on whether a real shell/`python3` exists in the environment.
   */
