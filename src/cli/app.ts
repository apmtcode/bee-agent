...
export type OperatorCliAppOptions = {
   ...
   * OS process in production; tests should pass a deterministic no-op spawner
   * so the suite never depends on real process scheduling or racing state-file
   * writes from the launch script.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Injectable process-liveness probe, mirrored from the runtime options. */
