...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background-task launcher processes are spawned. Defaults to a
   * real detached `spawn`. Tests inject a deterministic stub so the launcher
   * subprocess never races the test's own state writes.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Override liveness detection for background-task PIDs (mirrors the runtime option). */
