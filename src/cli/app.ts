...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional hooks forwarded to the background-task runtime. Tests inject a
   * no-op spawn so background tasks never launch real detached OS processes
   * (which would asynchronously rewrite state files and make assertions flaky).
   * Production leaves these undefined and uses the real `spawn`.
   */
