...
export type OperatorCliAppOptions = {
  ...
  /**
   * Overrides how background-task worker processes are spawned. Defaults to a
   * real `child_process.spawn`. Tests should inject a deterministic stub so no
   * real OS process is launched (a launched worker writes the same state/output
   * files the runtime reads, which races with any test that writes them by hand).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /**
   * Overrides how background-task process liveness is probed. Defaults to a real
   * `process.kill(pid, 0)`. Tests should inject a deterministic stub so recovery
   * outcomes do not depend on whether a real worker is still alive.
   */
