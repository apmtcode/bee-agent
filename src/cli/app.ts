...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background tasks are launched. Defaults to spawning a real
   * detached OS process. Inject a simulated spawn (see
   * `createSimulatedBackgroundSpawn`) to drive the app deterministically in
   * tests or sandboxes where real process launches are unavailable or flaky.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /**
   * Override the process-liveness probe used to reconcile background tasks.
   * Defaults to a real `process.kill(pid, 0)` check.
   */
