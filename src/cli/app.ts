...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injectable background-task process launcher. Defaults to the runtime's real
   * `spawn`-backed implementation in production. Tests should pass a deterministic
   * in-process fake so the suite never depends on real subprocess timing.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Injectable liveness probe paired with {@link backgroundTaskSpawnProcess}. */
