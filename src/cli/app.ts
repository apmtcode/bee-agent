...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injectable spawn for background tasks. Defaults to the real `spawn` in
   * production; tests pass a stub so no detached OS process is launched (whose
   * async state writes would otherwise race with the test's assertions).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Injectable liveness probe for background-task recovery; see the option above. */
