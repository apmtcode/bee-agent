...
export type OperatorCliAppOptions = {
  ...
  /**
   * Backend used to launch background-task processes. Defaults to a real
   * `child_process.spawn` in production. Tests (and any embedding that must not
   * fork real OS processes) inject a deterministic stub so background-task
   * bookkeeping can be exercised without racing a live subprocess.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Predicate used to probe whether a tracked background PID is still alive. */
