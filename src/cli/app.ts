...
export type OperatorCliAppOptions = {
  ...
  configHome?: string;
  /**
   * Overrides how background-task launch scripts are spawned. Production spawns a
   * real detached process; tests inject a deterministic no-op so the child's
   * asynchronous state write never races the assertions.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Overrides the liveness probe for background-task pids. Tests inject a
   * deterministic value so recovery does not depend on real process scheduling. */
