...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional override for how background tasks spawn their launch process.
   * Production uses the real `spawn`; tests inject a deterministic mock so no
   * real bash launch script runs asynchronously and races state assertions.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Optional override for the liveness check of a background task's process. */
