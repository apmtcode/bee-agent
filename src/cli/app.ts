...
export type OperatorCliAppOptions = {
  ...
  /**
   * Backend that launches background-task processes. Injectable so tests can
   * supply a deterministic fake (returning a stable pid without spawning a real
   * OS process), keeping the suite hermetic and free of orphaned processes and
   * racy state-file writes. Defaults to the runtime's real `spawn`-based backend.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /**
   * Predicate used to decide whether a background task's pid is still alive.
   * Injectable for the same hermeticity reasons as `backgroundTaskSpawnProcess`.
   */
