...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional injection points for the background-task execution backend.
   * Production leaves these undefined (real OS spawn + liveness probe). Tests
   * that drive execution state deterministically inject a no-op spawn so a real
   * launch process never races their state writes — mirrors `configHome`.
   */
