...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional overrides for how background tasks are spawned / probed. Production
   * uses the real `spawn`; tests inject a deterministic no-op spawn so
   * background-task state is governed solely by explicit `writeState` calls and
   * never races a real subprocess.
   */
