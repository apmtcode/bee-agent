...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background tasks are spawned. Production uses a real detached
   * child process; tests inject a deterministic stub so no real OS process is
   * launched (and long-running commands like `tail -f` never hang the suite).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Override the liveness probe for background task PIDs (test hermeticity). */
