...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background tasks are launched. Defaults to a real detached
   * process. Tests inject a no-op spawn so the runtime never launches a real
   * subprocess whose async state writes would race manual state fixtures.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /**
   * Override the process-liveness probe for background-task recovery. Defaults
   * to a real `process.kill(pid, 0)` check.
   */
