...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background tasks spawn their detached launcher process. Tests
   * that drive execution state explicitly should inject a no-op spawn (see
   * `createNoopBackgroundSpawn`) so no real shell process is launched.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Override background-task liveness detection (defaults to a real `process.kill` probe). */
