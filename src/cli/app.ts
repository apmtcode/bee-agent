...
export type OperatorCliAppOptions = {
  ...
  configHome?: string;
  /**
   * Injectable background-task process spawner. Defaults to the runtime's real
   * `spawn`. Tests should pass an inert spawner so no detached OS process is
   * launched (which would asynchronously mutate task state and race the test).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Injectable liveness check for a background-task pid. Defaults to `process.kill(pid, 0)`. */
