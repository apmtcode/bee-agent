...
export type OperatorCliAppOptions = {
  ...
  /**
   * How background-task launch scripts are spawned. Defaults to a real detached
   * child process. Tests should inject a no-op stub so no real process is
   * spawned and races the test's own state writes.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /**
   * How liveness of a background-task pid is probed. Defaults to a real
   * `process.kill(pid, 0)` check.
   */
