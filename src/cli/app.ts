...
export type OperatorCliAppOptions = {
   ...
   * `spawn` in production; tests pass a no-op so no real OS process is launched
   * (and no launch script races the test's own state writes).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Injectable liveness probe for a spawned background task's pid (test seam). */
