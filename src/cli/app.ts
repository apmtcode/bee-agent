...
export type OperatorCliAppOptions = {
  ...
  /**
   * Overrides for how background tasks spawn / probe OS processes. Production
   * leaves these unset (real `spawn`/`process.kill`). Tests inject a no-op
   * spawn (and a deterministic liveness probe) so background-task assertions do
   * not race a real detached child.
   */
