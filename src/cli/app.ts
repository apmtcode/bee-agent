...
export type OperatorCliAppOptions = {
  ...
  configHome?: string;
  /**
   * Background-task spawn/liveness hooks forwarded to the runtime. Production
   * leaves these undefined (real `spawn` + `process.kill(0)` probing); tests
   * inject deterministic stubs so background-task assertions don't depend on
