...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional override for how background tasks spawn their OS process. Production
   * uses the real detached spawn; tests inject a deterministic fake so the CLI
   * suite never depends on real process liveness or timing.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /**
   * Optional override for the background-task liveness probe. Pairs with
   * {@link backgroundTaskSpawnProcess} to make background-task reconciliation
