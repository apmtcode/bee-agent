...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injectable background-task process spawner. Production uses the real
   * `child_process.spawn`; tests pass a stub so no OS process is launched and
   * execution state is driven deterministically instead of racing a detached
   * launch script.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Injectable liveness probe for background-task processes (tests stub this). */
