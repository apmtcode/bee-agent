...
export type OperatorCliAppOptions = {
  ...
  /**
   * Injectable background-task process spawner. Defaults to a real OS spawn in
   * production; tests pass a deterministic mock so background tasks never launch
   * real processes (which would otherwise race the recorded execution state and
   * make health/recovery assertions flaky).
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Injectable liveness probe for background-task pids; see the spawner above. */
