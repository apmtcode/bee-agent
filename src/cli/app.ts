...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background-task launch scripts are spawned. Injected in tests
   * so background-task execution stays hermetic (no real detached OS process),
   * mirroring the seam already exposed by `StandaloneOperatorRuntime`.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Override liveness probing for background-task processes (tests). */
