...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background-task subprocesses are spawned. Production uses a
   * real detached OS process; tests inject a deterministic spawner (see
   * `harness/background-tasks.testkit`) to avoid real-subprocess flakiness.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Override how background-task process liveness is probed (tests). */
