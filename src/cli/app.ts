...
export type OperatorCliAppOptions = {
  ...
  /**
   * Background-task process seams. Production leaves these undefined (real
   * `child_process.spawn` + live PID probe). Tests inject deterministic fakes so
   * the unit suite never spawns a real OS process — spawning one races the
   * launch script's non-atomic state write and makes results host-timing
   * dependent.
   */
