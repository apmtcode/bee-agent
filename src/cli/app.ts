...
export type OperatorCliAppOptions = {
  ...
  /**
   * Optional override for how background tasks spawn their detached process.
   * Production leaves this unset (real `spawn`). Tests inject an inert spawn
   * (see `createInertBackgroundSpawn`) so execution state is driven explicitly
   * instead of racing a real subprocess.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Optional override for the background-task liveness probe (see above). */
