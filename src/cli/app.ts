...
export type OperatorCliAppOptions = {
   ...
   * real detached `spawn`. Tests inject a no-op so control-plane behaviour is
   * deterministic instead of racing the detached launch script's state writes.
   */
  backgroundTaskSpawnProcess?: ConstructorParameters<typeof StandaloneOperatorRuntime>[0]["backgroundTaskSpawnProcess"];
  /** Override the liveness probe used for recovered background tasks. */
