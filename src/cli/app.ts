...
export type OperatorCliAppOptions = {
  ...
  /**
   * Overrides for the background-task execution backend. Production leaves these
   * undefined so real detached processes are spawned and probed via `kill(0)`.
   * Tests should inject deterministic stubs so the suite never depends on real
   * subprocess timing (a launch wrapper exiting mid-assertion previously flipped
   * `isProcessRunning` and made platform-control state flaky).
   */
