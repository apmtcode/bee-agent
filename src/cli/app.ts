...
export type OperatorCliAppOptions = {
  ...
  /**
   * Override how background-task processes are spawned. Defaults to the
   * runtime's real `child_process.spawn`. Tests inject a deterministic no-op
   * spawner so background tasks never race real OS processes.
   */
  backgroundTaskSpawnProcess?: StandaloneOperatorOptions["backgroundTaskSpawnProcess"];
  /** Override background-task liveness checks (defaults to a real signal probe). */
