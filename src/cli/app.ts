...
export type OperatorCliAppOptions = {
   ...
   * undefined so the runtime uses the real `spawn`/`process.kill`. Tests inject
   * an inert spawn so no real detached process runs and the recorded task state
   * is authoritative (deterministic, hermetic runs).
   */
