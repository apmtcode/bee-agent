...
// A spawn stub that never launches a real OS process. Background-task tests drive
// execution state manually, so they must not race with — or leak — real detached
// processes whose launch scripts write state files asynchronously.
...
function makeApp(options: ConstructorParameters<typeof OperatorCliApp>[0]): OperatorCliApp {
  return new OperatorCliApp({ backgroundTaskSpawnProcess: noopSpawn, ...options });
}
...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // This test exercises the *real* background-task launch pipeline end to end
    // (`printf ok` → launch script → state/output files), so it must use the
    // real spawn rather than the noopSpawn stub the other app tests share.
