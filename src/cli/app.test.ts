...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // This test drives real end-to-end background execution (live `printf`
    // output, active-task watch), so it intentionally uses the real spawn
    // rather than the inert one used elsewhere.
