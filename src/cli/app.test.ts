...
import { createInertBackgroundSpawn } from "../harness/background-tasks.testing.js";
...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    const rootDir = await makeTempDir();
    const inertSpawn = createInertBackgroundSpawn();
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
      backgroundTaskSpawnProcess: inertSpawn.spawn,
      backgroundTaskIsProcessRunning: inertSpawn.isProcessRunning,
    });
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    // The inert spawner never runs the real command, so record the execution
    // state and output deterministically (the process pid it handed out is
    // reported as alive, keeping the task active for the watch-active check).
