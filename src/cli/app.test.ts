...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    const rootDir = await makeTempDir();
    // Hermetic background execution (see notes above): stub spawn so tasks get
    // the deterministic pid 4242, and treat only that pid as alive. This keeps
    // healthy tasks "active" without host-dependent subprocess timing, while
    // still letting the test degrade a task by writing a foreign (dead) pid.
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Hermetic background execution: stub the spawn so no real detached process
    // runs, and report the process as alive so status/sync stay deterministic
    // (real subprocess timing + process-group probes are host-dependent).
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
    ...
    // Supply the output the stubbed `printf ok` process would have written.
