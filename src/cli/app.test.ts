...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      cwd: rootDir,
      currentDate: "2026-05-25",
      // Simulate the background spawner so no detached OS process races the
      // output/state assertions below; keep tasks "running" deterministically.
      backgroundTaskSpawnProcess: createSimulatedBackgroundSpawn(),
    ...
    // The simulated spawner does not run the command, so stand in for the
    // process the way the launch script would: emit its output and a running
    // execution state that the sync/view/watch commands reconcile against.
