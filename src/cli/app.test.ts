...
import { createInertBackgroundSpawn } from "../harness/inert-spawn.js";

const inertBackgroundSpawn = createInertBackgroundSpawn();
...
describe("OperatorCliApp", () => {
  ...
  it("shows, configures, and disables the status line command", async () => {
    ...
    const app = new OperatorCliApp({ rootDir, cwd: rootDir, currentDate: "2026-05-25", configHome, backgroundTaskSpawnProcess: inertBackgroundSpawn });
  ...
  it("updates the live status line controller when slash commands change config", async () => {
    ...
    stdout.isTTY = true;
    stdout.columns = 80;
    const app = new OperatorCliApp({ rootDir, cwd: rootDir, currentDate: "2026-05-25", stdout, backgroundTaskSpawnProcess: inertBackgroundSpawn });
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Inert spawn + always-alive liveness: this test exercises the CLI command
    // surface (list/sync/view/watch/stop) deterministically. It does NOT rely on
    // a real detached `printf ok` finishing at a particular instant — an instant
    // command would race the "active task" assertions. The real launch pipeline
    // (and its JSON-safety) is covered by background-tasks.test.ts instead.
    ...
    // The inert spawn runs nothing, so seed the captured output the CLI reads.
