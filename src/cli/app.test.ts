...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      cwd: rootDir,
      currentDate: "2026-05-25",
      // Inert spawn + always-alive probe make this an integration test over the
      // command surface without depending on real detached-process timing. The
      // task's output is written explicitly below (mirroring the monitor half of
      // this test) so `background-view`/`watch-task` never race the launch script.
      backgroundTaskSpawnProcess: createInertSpawn(),
