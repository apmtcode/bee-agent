...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    const rootDir = await makeTempDir();
    // Side-effect-free spawn so background tasks never launch a real subprocess
    // that would race the explicit writeState calls below. Until a test writes
    // state, a task simply has no state file — which the status logic treats as
    // healthy — so the platform/remote status assertions are deterministic.
