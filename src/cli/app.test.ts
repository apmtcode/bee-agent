...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      // Inert spawn so background tasks started below don't launch real detached
      // subprocesses that would race with the platform-control state assertions.
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      cwd: rootDir,
      currentDate: "2026-05-25",
      // Inert spawn + always-running probe so background/monitor task commands
      // don't launch real detached subprocesses that would race with this test's
      // task-state assertions. State/output are seeded explicitly below.
      backgroundTaskSpawnProcess: () => ({ pid: 4242, unref() {} }),
    ...
    // Seed a deterministic running state + output for the task (the inert spawn
    // means no real launch script runs), so the sync/view/watch/watch-active
    // assertions below don't race a fast-completing real `printf ok`.
