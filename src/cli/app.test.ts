...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      // Stub spawn/liveness so background-task commands don't launch real
      // detached processes that would race the test's own state writes.
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      currentDate: "2026-05-25",
      // Stub spawn/liveness so background-task commands don't launch real
      // detached processes that would race the test's own state writes. This
      // test exercises *active* task/monitor watching, so report the stubbed
      // process as alive (recovery-to-failed is covered elsewhere).
      backgroundTaskSpawnProcess: () => ({ pid: 4321, unref() {} }),
