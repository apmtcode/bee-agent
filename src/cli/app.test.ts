...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      // Deterministic no-op spawner: background tasks never launch real OS
      // processes, so state files are driven solely by explicit writeState
      // calls below (otherwise a detached shell races the control-state asserts).
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // No-op spawner keeps the task in "running" state; output is written
    // explicitly below (mirroring the monitor half of this test) so neither the
    // "active task" nor the "ok" output assertion races a real detached process.
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
