...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    const rootDir = await makeTempDir();
    const app = new OperatorCliApp({ rootDir, cwd: rootDir, currentDate: "2026-05-25", backgroundTaskSpawnProcess: () => ({ pid: 4321, unref() {} }), backgroundTaskIsProcessRunning: () => false });
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Inert spawn so no real detached process is launched; the task's output and
    // terminal state are written explicitly below. This removes the race where
    // the CLI read the output/state file before the real process had flushed it
    // (previously flaky), while still exercising the full command surface.
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
      backgroundTaskSpawnProcess: () => ({ pid: 4321, unref() {} }),
      // Treat the (inert) task's pid as alive so sync keeps it "running" and it
      // remains discoverable by watch-active / task-stop below.
    ...
    // Deterministically stand in for the output the real launch script would
    // have produced (command: `printf ok`). Keep the task "running" so the
    // active-watch and stop assertions below still apply.
