...
import { createDeterministicBackgroundSpawn } from "../harness/background-tasks.testkit.js";
...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      backgroundTaskSpawnProcess: createDeterministicBackgroundSpawn(),
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Inject a deterministic no-op spawn and a fixed liveness probe so the task
    // stays "running" until explicitly stopped, instead of racing a real
    // `printf` process that may exit before `task-stop` is dispatched.
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
    ...
    // The no-op spawn writes no output/state; emit the deterministic output the
    // command would have produced and pin a running state for reconciliation.
