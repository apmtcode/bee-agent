...
import { createSimulatedBackgroundProcess } from "../harness/simulated-process.js";
...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      backgroundTaskSpawnProcess: createSimulatedBackgroundProcess().spawn,
    });
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    const processBackend = createSimulatedBackgroundProcess();
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
      backgroundTaskSpawnProcess: processBackend.spawn,
      backgroundTaskIsProcessRunning: processBackend.isProcessRunning,
    });
    ...
    // The simulated process backend does not run `printf ok`, so stand in for
    // the execution output the task would have produced on a real machine.
