...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      // Keep launched tasks deterministically "running": stub the launcher so no
      // real detached subprocess runs, and report pids as live — except the
      // sentinel pid 999999 the test writes to force one task into the degraded
      // path. Otherwise a fast command (e.g. `printf`) exits before the
      // platform-status assertions, racing `control=active`/`degraded`.
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      cwd: rootDir,
      currentDate: "2026-05-25",
      // Stub the launcher: synchronously write the task's stdout ("ok") the way
      // the real `printf ok` command would, but without a detached subprocess
      // whose async writes and instant exit race the deterministic assertions
      // (the task must still read as "running" for watch-active).
      backgroundTaskSpawnProcess: (command) => {
        writeFileSync(path.join(path.dirname(command), "output.log"), "ok\n");
