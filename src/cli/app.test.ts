...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Deterministic background execution: a no-op spawn plus an always-alive
    // process check, with the task's output/state seeded explicitly below. A
    // real detached launch would race these assertions on task output/liveness.
    let spawnPid = 70000;
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
    ...
    await app.runtime.backgroundTasks.executionService.writeState(task, {
      ...
      kind: "task",
      status: "running",
      pid: task.execution.processId ?? spawnPid,
