...
import { createFakeBackgroundProcessBackend } from "../harness/fake-background-process.js";
...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    const rootDir = await makeTempDir();
    const backgroundProcesses = createFakeBackgroundProcessBackend();
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    // The fake process backend never launches the real `printf ok` script, so
    // record its output deterministically and keep it running (its fake pid is
    // reported alive by the backend) instead of racing a detached subprocess.
    // This keeps the task "active" for the watch/stop assertions below.
