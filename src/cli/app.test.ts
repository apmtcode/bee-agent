...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      // Deterministic background-task seam so status assertions never race a
      // real OS process's async state writes. pid 999999 is this test's
      // dead-process sentinel used to drive the "degraded" scenario below.
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      cwd: rootDir,
      currentDate: "2026-05-25",
      // Deterministic seam: stub the spawn so no real process races the manual
      // output/state writes, and keep launched tasks "alive" so the task stays
      // active for the watch assertions below.
      backgroundTaskSpawnProcess: () => ({ pid: 4242, unref() {} }),
    ...
    // The stubbed spawn does not run the real command, so drive the output the
    // view/watch assertions expect (mirrors how the monitor output is written).
