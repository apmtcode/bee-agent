...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    const rootDir = await makeTempDir();
    // Deterministic spawn: this test drives background-task state explicitly via
    // writeState, so a real detached process would only race its async launch
    // state write against these assertions under full-suite load.
