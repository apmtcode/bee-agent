...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    // Seed the execution artifacts deterministically instead of depending on the
    // real detached `printf ok` process (which under parallel-suite load can
    // complete and flip the task out of "running" before these assertions, or
    // race reconcile's non-atomic state read). Mirrors the monitor block below.
