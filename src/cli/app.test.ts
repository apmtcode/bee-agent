...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      backgroundTaskSpawnProcess: () => ({ pid: 5252, unref() {} }),
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      cwd: rootDir,
      currentDate: "2026-05-25",
      // Mock the spawner so background tasks don't launch real detached
      // subprocesses (which would race the store's persisted state). The task is
      // treated as a live process until it is explicitly stopped below.
      backgroundTaskSpawnProcess: () => ({ pid: 5252, unref() {} }),
    ...
    // Deterministically simulate the running process's effects (output + state)
    // that the real launch script would produce on disk.
    ...
    await app.runtime.backgroundTasks.executionService.writeState(task, {
      ...
      kind: "task",
      status: "running",
      pid: task.execution.processId ?? 5252,
