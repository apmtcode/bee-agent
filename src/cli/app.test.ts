...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Deterministic launch: don't spawn a real detached process (whose async
    // writes race these assertions). The stub reports a fixed pid, that pid is
    // treated as "alive", and the test drives execution output/state directly —
    // exactly as the monitor section below already does.
    const stubPid = 43210;
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
      backgroundTaskIsProcessRunning: (pid) => pid === stubPid,
      backgroundTaskSpawnProcess: () => ({ pid: stubPid, unref() {} }),
    });
    ...
    await app.runtime.backgroundTasks.executionService.writeState(task, {
      ...
      kind: "task",
      status: "running",
      pid: task.execution.processId ?? stubPid,
