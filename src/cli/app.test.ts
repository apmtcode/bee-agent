...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Inert spawn + always-running probe so the background task lifecycle is
    // driven by explicit writes below instead of racing a real detached shell
    // (a fast `printf` would otherwise complete before `watch-active` runs).
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
    ...
    // Simulate the command's stdout deterministically (mirrors the monitor path
    // below), since the inert spawn launches no real process.
