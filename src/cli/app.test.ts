...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Inject a deterministic spawn + liveness so no real detached subprocess
    // races with the test-managed state (the real launcher writes its own state
    // file asynchronously, which under parallel suite load clobbers the state we
    // set below). Execution output/state is written explicitly, mirroring the
    // monitor half of this test.
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
