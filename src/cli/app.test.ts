...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      // No-op spawner so the background task's launch script can't race the
      // test's own writeState() calls that drive the degraded-status assertions.
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      cwd: rootDir,
      currentDate: "2026-05-25",
      // No-op spawner + always-running probe so the task stays deterministically
      // "running": a real short-lived `printf ok` would exit before task-stop and
      // flip the message from "Stopped task" to "is not running". Output and state
      // are driven explicitly below, mirroring the monitor half of this test.
      backgroundTaskSpawnProcess: () => ({ pid: 4242, unref() {} }),
