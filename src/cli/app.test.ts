...
import { createSimulatedBackgroundSpawn } from "../harness/simulated-spawn.js";
...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Use a simulated spawn so no real detached subprocess races the task
    // state/output files; the lifecycle below is driven explicitly instead.
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
      backgroundTaskSpawnProcess: createSimulatedBackgroundSpawn(),
      // Treat the simulated task's pid as a live process so reconciliation keeps
      // an explicitly-"running" task running (instead of flipping it to failed).
    ...
    // Drive the task lifecycle deterministically: output present, still running
    // (so it stays the session's active task) until the explicit task-stop.
