...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Hermetic background-task backend: never launch a real OS process. The
    // task is treated as alive (`isProcessRunning: () => true`) and its
    // mid-flight output/state are seeded explicitly below, so the assertions
    // observe a deterministic "running with output" snapshot instead of racing
    // a real `printf ok` process.
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
    ...
    // Seed the mid-flight snapshot the (now mocked) launch script would have
    // produced: task still running, with its command output already flushed.
    await app.runtime.backgroundTasks.executionService.writeOutput(task, "starting task smoke\nok\n");
