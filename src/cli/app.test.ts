...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      // Never spawn a real background process; keep remote-control health
      // derivation deterministic (no racing "missing-process" from a launch script).
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      cwd: rootDir,
      currentDate: "2026-05-25",
      // Hermetic background tasks: never spawn a real short-lived process (whose
      // exit would race the stop/watch assertions below). The task stays
      // "running" (isProcessRunning => true) and its output is written
      // explicitly so view/watch remain deterministic under parallel load.
      backgroundTaskSpawnProcess: () => ({ pid: 424242, unref() {} }),
    ...
    // The mock spawn writes no output; record it explicitly so the view/watch
    // assertions below observe deterministic content.
