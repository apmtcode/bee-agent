...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      // No-op spawner: background tasks must not launch real OS processes whose
      // async state writes would race with the control-state assertions below.
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      cwd: rootDir,
      currentDate: "2026-05-25",
      // No-op spawner keeps execution state deterministic; the test seeds the
      // task's output/state directly (mirroring the monitor half below) instead
      // of racing a real detached `printf` process. isProcessRunning=true keeps
      // the seeded "running" task active (as a live process would) so the
      // watch-active/watch-task assertions resolve deterministically.
      backgroundTaskSpawnProcess: () => ({ pid: 4242, unref() {} }),
