...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      // Deterministic: don't launch real OS processes (e.g. `sleep 5`) whose
      // async state writes would race the remote-status assertions below.
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      cwd: rootDir,
      currentDate: "2026-05-25",
      // Deterministic: never launch a real OS process. The task's output/state
      // are driven explicitly below (mirroring the monitor block), so view/stop
      // don't race a fast `printf` that may exit before the stop call.
      backgroundTaskSpawnProcess: () => ({ pid: 4242, unref() {} }),
