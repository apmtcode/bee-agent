...
import { noopSpawnBackgroundProcess } from "../harness/background-tasks.js";
...
describe("OperatorCliApp", () => {
  it("dispatches status and runtime-backed commands", async () => {
    ...
    const app = new OperatorCliApp({ rootDir, cwd: rootDir, currentDate: "2026-05-25", backgroundTaskSpawnProcess: noopSpawnBackgroundProcess, backgroundTaskIsProcessRunning: () => false });
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // This test intentionally exercises the real launch pipeline (it asserts on
    // the subprocess's own "ok" output and on live-process detection), so it
    // uses the default spawner rather than the deterministic noop one.
