...
import { makeNoopSpawn } from "../harness/test-support.js";
...
describe("OperatorCliApp", () => {
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    ...
    const app = new OperatorCliApp({
      ...
      // Fake-spawned pids (>= 100000) are treated as alive; the test rewrites the
      // degraded task's state to the sentinel pid 999999 to simulate a dead
      // process, so only that pid probes as not-running.
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    ...
    // The spawner is a no-op in tests, so deterministically simulate what the
    // launched `printf ok` process would have written (its stdout + a running
    // execution state) instead of racing a real detached process.
    ...
    await app.runtime.backgroundTasks.executionService.writeState(task, {
      ...
      kind: "task",
      status: "running",
      pid: task.execution.processId ?? 100000,
