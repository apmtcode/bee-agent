...
/**
 * Deterministic background-task spawn stub: returns a fake pid without starting
 * a real detached OS process (whose async `state.json` write would race tests).
 */
function stubSpawn() {
...
describe("OperatorCliApp", () => {
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Stub the spawn (no real detached process) and treat its pid as live so the
    // task's output/state are driven deterministically below — mirroring how the
    // monitor half of this test writes its own artifacts. Without this the real
    // launcher races the assertions (output not yet flushed, or already exited so
