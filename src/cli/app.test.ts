...
// Deterministic background-task spawn so unit tests never fork real detached OS
// processes (whose async lifecycle otherwise races state assertions).
const BG_SPAWN_MOCK = () => ({ pid: 4321, unref() {} });
...
describe("OperatorCliApp", () => {
  it("dispatches status and runtime-backed commands", async () => {
    ...
    const app = new OperatorCliApp({ rootDir, cwd: rootDir, currentDate: "2026-05-25", backgroundTaskSpawnProcess: BG_SPAWN_MOCK });
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Uses the real spawn: this test asserts on the actual `printf ok` output
    // captured via /background view and /watch, so it must run the command.
