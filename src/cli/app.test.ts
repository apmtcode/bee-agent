...
describe("OperatorCliApp", () => {
  it("dispatches status and runtime-backed commands", async () => {
    ...
    const app = new OperatorCliApp({ rootDir, cwd: rootDir, currentDate: "2026-05-25", backgroundTaskSpawnProcess: () => ({ pid: 4321, unref() {} }) });
  ...
  it("supports session lifecycle, transcript, approvals, pairing, config, and prompt commands", async () => {
    const rootDir = await makeTempDir();
    // Inert background-task spawn so the launch script never runs — keeps this
    // test deterministic regardless of whether a real shell exists (otherwise a
    // real `printf`/`sleep` process finishes and reconciles as missing-process).
  ...
  it("supports background and monitor task commands plus cron commands", async () => {
    const rootDir = await makeTempDir();
    // Inert spawn + a live-process probe so the started task stays active for
    // the watch-active assertion (deterministic regardless of the real shell).
    const app = new OperatorCliApp({
      rootDir,
      cwd: rootDir,
      currentDate: "2026-05-25",
