...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial "running" state via Python so the JSON is always
    // well-formed regardless of what characters the command string contains.
    // (The previous `printf | sed` pipeline had broken shell quoting that
    // corrupted the state file — see renderRunningStateWriterPython.)
