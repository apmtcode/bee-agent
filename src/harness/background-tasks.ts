...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // The initial "running" state is written by python (not sed) so the real
    // PID ($$) is substituted correctly and the file is written atomically
    // (temp + os.replace). The previous sed approach embedded literal double
    // quotes inside a double-quoted argument, which broke the shell quoting and
    // left `pid` as the literal string "$$" — making every task look dead.
...
// Atomically write a JSON file: dump to `<name>.tmp` then os.replace() over the
// target, so a concurrent reader never observes a partially written file.
