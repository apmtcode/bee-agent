...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  const quotedOutputValue = shellQuote(task.execution.outputFile);
  ...
  return [
    ...
    // Build the initial "running" state with Python (already required below for
    // the terminal write) rather than shell-interpolating values into JSON: the
    // command/cwd can contain quotes and `$`, which the previous `sed` approach
    // corrupted into invalid JSON. Values arrive as argv, so no escaping is
    // needed, and the write is atomic (temp + os.replace) so a concurrent reader
    // never observes a half-written state.json.
    `python3 - ${quotedStatePath} "$$" "$started_at" ${quotedTaskId} ${quotedKind} ${quotedOutputValue} ${quotedCwd} ${quotedCommand} <<'PY'`,
...
function renderInitialStateWriterPython(): string[] {
  return [
    ...
    "    'command': sys.argv[8],",
    "}",
    "tmp_path = state_path.with_name(state_path.name + '.py.tmp')",
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic terminal-state write: render to a temp sibling then os.replace() it
    // into place (atomic on the same filesystem), matching writeJsonAtomic().
    "tmp_path = state_path.with_name(state_path.name + '.py.tmp')",
    "tmp_path.write_text(json.dumps(state, indent=2) + '\\n')",
    "os.replace(tmp_path, state_path)",
  ];
...
function shellQuote(value: string): string {
  // POSIX single-quote escaping: end the quote, emit an escaped quote as `"'"`,
  // then reopen — i.e. each `'` becomes `'"'"'`. The previous form used an extra
  // leading double-quote (`"'"'"'`), which reconstructed a command/cwd value
  // containing single quotes into malformed shell (e.g. `printf "'x'"`), so any
  // quoted task command exited non-zero and left a corrupt state.json behind.
