...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base state (without the runtime-resolved pid/timestamps). Passed to a small
  // python writer as a single shell-quoted argv so that commands containing
  // quotes, newlines, `$$`, or the timestamp sentinel round-trip losslessly —
  // the previous `printf | sed >` approach both mangled such commands and, due
  // to broken shell quoting in the `s/"$$"/…/` rule, never substituted the pid.
  const quotedInitialState = shellQuote(
    JSON.stringify({
      version: 1,
      taskId: task.id,
      kind: task.kind,
  ...
  return [
    ...
    `python3 - ${quotedStatePath} ${quotedInitialState} $$ "$started_at" <<'PY'`,
...
function renderInitialStateWriterPython(): string[] {
  return [
    ...
    "state['pid'] = int(sys.argv[3])",
    "state['startedAt'] = sys.argv[4]",
    "state['updatedAt'] = sys.argv[4]",
...
function shellQuote(value: string): string {
  // Wrap in single quotes and escape embedded single quotes with the POSIX
  // idiom `'\''` (close quote, escaped quote, reopen). The previous replacement
  // (`"'"'"'`) was malformed and corrupted any value containing an apostrophe —
  // e.g. a command like `printf 'hi'` — producing a broken launch script.
