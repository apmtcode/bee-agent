...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The initial "running" state is written by the same atomic Python writer used
  // for completion, with the dynamic fields (pid, timestamps) supplied as argv.
  // The static base — including the task command, which may contain arbitrary
  // quotes/newlines — is passed as one shell-quoted JSON argument and parsed by
  // `json.loads`, so no fragile in-shell `sed`/quote substitution is needed.
