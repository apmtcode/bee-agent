...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // reader (sync/health-check) never observes a truncated, half-written file.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic terminal-state write: write a sibling temp file then os.replace()
    // (atomic on the same filesystem) so readers never see a torn JSON document.
