...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The pid (bash $$) and startedAt (shell `date`) are filled in at runtime by
  // the python writer below, not by string substitution — embedding them as
  // placeholders and rewriting via `sed` was fragile: the inner quotes/`$$`
  // could not survive both the JS template literal and bash double-quoting, so
  // pid stayed the literal string "$$" and quoted commands corrupted the JSON.
...
function shellQuote(value: string): string {
  // Escape embedded single quotes by closing the quote, emitting a
  // double-quoted single quote, then reopening: '"'"'. (A leading double
  // quote here — "'"'"' — injects a stray literal '"' that corrupts any
  // quoted command, e.g. breaking the JSON written to state.json.)
