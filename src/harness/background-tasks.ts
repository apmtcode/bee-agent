...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Encode the running-state payload as base64 so the launch script can hand it to
  // python untouched: shell-quoting + `printf | sed` mangled commands containing
  // quotes (e.g. `printf 'x'`) into invalid JSON, and the plain `> file` redirect was
  // non-atomic, so a concurrent reader could observe a truncated file. python injects
  // the real pid/timestamps and writes via os.replace (atomic).
