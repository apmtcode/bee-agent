...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial "running" state via Python's json module so arbitrary
    // command/cwd strings (quotes, newlines, `$`) are safely serialized. Fields
    // are passed as argv, not interpolated into JSON text, so the launch script
    // can never emit an invalid state file. (Replaces a fragile `printf | sed`
    // string-munging approach that corrupted JSON for such commands.)
...
function shellQuote(value: string): string {
  // POSIX-safe single-quoting: close the quote, emit a double-quoted single
  // quote (`"'"`), then reopen — i.e. each `'` becomes `'"'"'`. The previous
  // `"'"'"'` had a stray leading `"`, which mangled any command/path/cwd
  // containing a single quote (e.g. `printf 'x'`), so the launched process ran
  // a corrupted command and the persisted state recorded the wrong command.
