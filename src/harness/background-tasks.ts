...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base state payload with no pid/timestamps; the initial-state writer below
  // fills those in via Python. This avoids the previous `printf | sed` pipeline,
  // whose `s/"$$"/…/` substitution silently never matched ($ is a sed anchor)
  // and which corrupted the JSON whenever the command contained quotes/newlines.
