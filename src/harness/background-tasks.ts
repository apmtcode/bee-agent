...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial running-state via python3 (like the completion path) so
    // metadata containing quotes/newlines cannot corrupt the JSON, and the pid
    // is written as a real number rather than substituted via a fragile sed.
...
function shellQuote(value: string): string {
  // POSIX-safe single-quoting: wrap in single quotes and replace every embedded
  // single quote with the `'"'"'` idiom (close-quote, a double-quoted quote,
  // reopen-quote). The previous `"'"'"'` ordering was reversed and corrupted any
  // command/path containing a single quote (e.g. `printf 'x'`).
