...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base payload with placeholder pid/timestamps; `printf` writes it verbatim and
  // python (below) patches the real pid ($$) and start time. We deliberately avoid
  // the previous sed-based injection: `$$` is a regex anchor and its shell
  // expansion inside a double-quoted sed program is environment-dependent, which
  // could emit invalid JSON (an unquoted `"pid":$$`). Having python read the file
  // and re-serialize it is robust across shells.
...
function shellQuote(value: string): string {
  ...
  // reopen. The replacement is `'\''` — the previous `"'"'"'` form injected a
  // spurious `"` for every single quote, corrupting any command/path/JSON that
  // contained one (e.g. `printf 'ok'`), which broke the launch script's state.json.
