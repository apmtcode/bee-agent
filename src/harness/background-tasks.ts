...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base state payload is emitted as a single JSON argument and parsed by
  // python3; runtime-only fields (pid, timestamps) are injected by the writer.
  // We deliberately avoid `printf | sed` string munging here — sed's quoting
  // collapses under bash expansion and corrupts any command containing quotes,
  // producing invalid JSON (and leaving `pid` as the literal "$$").
...
function renderStateInitializerPython(): string[] {
  return [
    "import json",
    "import os",
    "import pathlib",
...
function shellQuote(value: string): string {
  ...
  // form `"'"'"'` was not a valid escape and corrupted any value containing a
  // single quote — breaking the emitted state JSON and command execution.
