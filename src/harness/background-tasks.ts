...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial "running" state atomically: render to a temp file and
    // rename it into place so a concurrent reader (the reconciler, remote-status
    // diagnostics, or a test) never observes a half-written, unparseable file.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: render to a sibling temp file then os.replace() (atomic
    // rename) so readers never see a truncated terminal-state file.
