...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Substitute the started-at placeholder and the literal `"$$"` pid placeholder
    // with the real shell pid (numeric), then rename into place so readers never
    // observe a torn write. The search pattern must reach sed as a literal `"$$"`,
    // hence the doubled backslashes in this template literal.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    "tmp_path = state_path.with_name(state_path.name + '.writing')",
...
function shellQuote(value: string): string {
  ...
  // A single quote `'` becomes `'"'"'` (end-quote, double-quoted quote, start-quote).
