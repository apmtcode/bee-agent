...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base running-state payload without the runtime-derived pid/timestamps. The
  // launch script injects those via python3 (below) rather than shell string
  // substitution — passing the JSON as a python argv value is immune to the
  // shell-quoting pitfalls that a `sed` placeholder swap is prone to.
...
function shellQuote(value: string): string {
  // POSIX single-quote escaping: a single quote cannot appear inside a
  // single-quoted string, so each `'` is emitted as `'"'"'` — close the quote,
  // a double-quoted literal quote, then reopen. (The previous sequence
  // `"'"'"'` was off by one and round-tripped `'foo'` to `"'foo"'`, corrupting
  // any command or JSON payload that contained a single quote.)
