...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // template so that arbitrary command strings (quotes, newlines, `$$`, etc.)
  // cannot corrupt the file the way a `printf | sed` pipeline could.
...
function shellQuote(value: string): string {
  // Close the quote, emit a double-quoted literal quote, reopen: 'foo'"'"'bar'.
