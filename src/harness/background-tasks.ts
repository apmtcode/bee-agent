...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base "running" state as a single-line JSON literal. pid/startedAt are filled
  // in by python at launch time (via argv), so the command — which may contain
  // single quotes and newlines — round-trips through JSON rather than a fragile
  // sed placeholder substitution that corrupts quotes and leaves `pid` unset.
  ...
  return [
    ...
    // Atomic write of the initial "running" state (write temp, then os.replace)
    // so a concurrent reconcile/sync reader never observes a half-written or
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Write atomically: a reconcile/sync reader can run concurrently with this
    // writer, so never expose a truncated state.json. Write a sibling temp file
    // then os.replace() it into place (atomic rename on POSIX + Windows).
    "tmp_path = state_path.with_name(state_path.name + '.tmp')",
    "tmp_path.write_text(json.dumps(state, indent=2) + '\\n')",
    "import os",
    "os.replace(tmp_path, state_path)",
...
function shellQuote(value: string): string {
  ...
  // canonical `'\''` sequence (close-quote, escaped literal quote, reopen-quote).
  // The previous `"'"'"'` sequence was malformed and corrupted every value that
  // contained a single quote — e.g. `printf 'hi'` became `printf "'hi"'` — which
  // broke both the executed command and the JSON state payload.
