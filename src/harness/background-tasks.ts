...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base state; pid/startedAt/updatedAt are filled in at runtime by python so we
  // never string-substitute into JSON (sed mangles values containing quotes or
  // backslashes, e.g. a command like `printf "x\n"`, producing invalid JSON).
  ...
  return [
    ...
    // Build the initial state with python (robust JSON + atomic replace) rather
    // than a fragile printf|sed redirect.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic replace so readers never see a partially rewritten state file.
    "tmp_path = state_path.with_name(state_path.name + '.tmp')",
    "tmp_path.write_text(json.dumps(state, indent=2) + '\\n')",
    "os.replace(tmp_path, state_path)",
  ];
