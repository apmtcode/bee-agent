...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Build the initial "running" state in python (from argv) rather than
    // sed-substituting into a hand-built JSON string: sed on a JSON blob mangles
    // commands containing quotes and cannot reliably inject the pid, producing
    // invalid JSON that a concurrent reconcile read then fails to parse. Passing
    // values as argv keeps the JSON valid for any command content, and the
    // temp-file + rename makes the write atomic against concurrent readers.
...
function shellQuote(value: string): string {
  ...
  // the quoted string, emitting an escaped quote via a double-quoted section,
  // then reopening — i.e. each `'` becomes `'"'"'`. (The previous `"'"'"'`
  // sequence was malformed and left commands containing single quotes — e.g.
  // `printf 'foo'` — with unbalanced quotes, so they failed to execute.)
