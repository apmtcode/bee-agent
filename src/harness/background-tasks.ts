...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Build the initial "running" state with python's json.dumps rather than
    // shell string-templating: the task command may contain quotes, newlines,
    // or other JSON-significant characters, so hand-quoting it into a JSON blob
    // through the shell produces invalid JSON. Passing every field as a distinct
    // argv value keeps the serialization robust for any command string.
