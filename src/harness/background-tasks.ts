...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base state payload with pid/startedAt/updatedAt injected at runtime. We pass
  // this JSON as a single shell argument to python3 rather than mutating it with
  // sed: the payload may contain shell/JSON metacharacters (quotes, newlines,
  // `$`) from `task.command`, and text substitution on it produced invalid JSON.
...
function shellQuote(value: string): string {
  ...
  // idiom `'"'"'` (close quote, quoted quote, reopen quote). The previous
  // replacement inserted a spurious double quote, corrupting any command — or
  // JSON payload — that contained a single quote.
