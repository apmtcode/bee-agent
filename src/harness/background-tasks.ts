...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Build the initial "running" state via python's json.dumps rather than
    // sed-templating a hand-built string: task.command / cwd may contain quotes,
    // newlines, backslashes or sed metacharacters that would otherwise corrupt
    // the JSON. All dynamic values are passed as argv and escaped by json.dumps.
