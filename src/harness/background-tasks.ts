...
function shellQuote(value: string): string {
  // POSIX single-quote escape: close the quote, emit a double-quoted literal
  // quote, reopen — `'"'"'`. The previous `"'"'"'` variant was malformed and
  // produced an invalid JSON state file whenever a task command (or cwd/id)
  // contained a single quote, which then broke state reads during recovery.
