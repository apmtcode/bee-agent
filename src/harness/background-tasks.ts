...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial "running" state with python/json.dumps (as the
    // completed/failed writers below do) so any characters in the command —
    // quotes, newlines, backslashes — are serialized safely. The previous
    // printf|sed placeholder substitution mangled such payloads.
...
function shellQuote(value: string): string {
  ...
  // embedded single quote with '"'"' (close-quote, a double-quoted single
  // quote, reopen-quote). The sequence must START with a single quote — a
  // leading double quote (`"'"'"'`) corrupts the reconstruction for any value
  // containing a single quote (e.g. a shell command like `printf 'x'`).
