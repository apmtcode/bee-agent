...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The running-state payload is passed to python as a single shell-quoted
  // argument and re-serialized via json.dumps. This keeps the state file valid
  // JSON regardless of quotes/newlines in the command — unlike the previous
  // `printf | sed` pipeline, which corrupted the file for commands containing
  // quotes and silently failed to substitute the pid. pid/startedAt/updatedAt
  // are injected in python so they reflect the real runtime values.
