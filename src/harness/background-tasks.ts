...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base state written to disk at launch. `pid`/`startedAt`/`updatedAt` are
  // placeholders filled in by the Python writer below at runtime — we never do
  // text substitution on the JSON itself, so a command containing quotes,
  // backslashes, `$`, or newlines can't corrupt the resulting state file.
...
function shellQuote(value: string): string {
  ...
  // double-quoted `'`, then reopen — i.e. every `'` becomes `'"'"'`. The prior
  // sequence began with a stray `"`, so any command containing a single quote
  // was mis-quoted, corrupting both `bash -lc <command>` execution and the
