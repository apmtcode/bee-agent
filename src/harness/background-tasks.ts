...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The initial "running" state is written by python3 (same interpreter used by
  // the completion writer below) rather than a `printf | sed` pipeline. The sed
  // approach mangled the JSON whenever the command contained double quotes: the
  // `s/"$$"/…/` expression was re-parsed by bash's own quoting, dropping the
  // escaping and leaving invalid JSON on disk. Passing the payload through argv
  // and parsing it with json.loads is quote-safe for arbitrary commands.
  const quotedInitialPayload = shellQuote(
    JSON.stringify({
      version: 1,
      taskId: task.id,
      kind: task.kind,
  ...
  return [
    ...
    `python3 - ${quotedStatePath} $$ "$started_at" ${quotedInitialPayload} <<'PY'`,
...
function shellQuote(value: string): string {
  ...
  // (`"'"`), then reopen. The literal must be `'"'"'` — a leading double quote
  // (`"'"'"'`) produces malformed output that corrupts any value containing a
  // single quote (e.g. `git commit -m 'msg'`).
