...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base64-encode the state template so arbitrary command text (embedded quotes,
  // newlines, shell metacharacters) round-trips into the state file as valid
  // JSON. The previous `sed`-into-JSON substitution corrupted the file for such
  // commands, which then crashed the Python completion writer and left a
  // malformed state.json behind.
...
function renderInitialStateWriterPython(): string[] {
  return [
    ...
    "pid = int(sys.argv[2])",
    "timestamp = sys.argv[3]",
    "state = json.loads(base64.b64decode(sys.argv[4]))",
    "state['pid'] = pid",
