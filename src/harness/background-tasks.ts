...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base running-state document. `pid`, `startedAt` and `updatedAt` use quote-free
  // placeholders so the sed substitution below never contains a `"` that would
  // close the double-quoted sed program. The old `s/"$$"/.../` form left the pid
  // as the literal string "$$" (an invalid pid), so a live task was misjudged as
  // `missing-process` for its whole running window. All string fields are already
  // JSON-escaped by JSON.stringify, so only these numeric/plain values are templated.
  ...
  const quotedRunningPayload = shellQuote(runningPayload);
  ...
  return [
    ...
    // Fast, un-failable shell write of the running state (a Python call here could,
    // under `set -e`, die before the command runs and orphan the task). Staged to a
    // temp file then renamed, so a concurrent reader never observes a torn file.
    `state_tmp="$state_path.$$.tmp"`,
    `printf '%s' ${quotedRunningPayload} | sed "s/__OPENCLAW_PID__/$$/g; s/__OPENCLAW_STARTED_AT__/$started_at/g" > "$state_tmp"`,
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Stage + atomic rename so a concurrent reader never observes a truncated file.
...
function shellQuote(value: string): string {
  ...
  // The sequence is '"'"' (quote, dq-quote-dq, quote) — NOT "'"'"' — otherwise
  // every embedded single quote is rewritten as a spurious double quote, which
  // corrupts the JSON the launch script writes for commands like `printf 'x'`.
