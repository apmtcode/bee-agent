...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base state (everything except the runtime-injected pid/startedAt/updatedAt).
  // Passed to python3 via an env var so arbitrary command text — single quotes,
  // double quotes, newlines — round-trips as raw bytes and cannot corrupt the
  // serialized JSON. (A prior `printf | sed` approach mangled shell-hostile
  // commands such as `printf 'line-1\nline-2\n'` into invalid JSON.)
...
function shellQuote(value: string): string {
  ...
  // double-quoted section, reopen. The sequence MUST start with `'` (close the
  // span) — a prior version used `"'"'"'`, whose leading `"` injected a spurious
  // double-quote before every single quote, corrupting quoted commands.
