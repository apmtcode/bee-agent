...
// Build the initial "running" state entirely in Python from a pre-serialized
// JSON blob. Embedding the command via shell `printf | sed` (the previous
// approach) corrupted the JSON whenever the command contained quotes or
// newlines, and the `s/"$$"/.../` substitution silently failed because `$` is a
// sed anchor — leaving `pid` as the literal string "$$". Passing a
// double-JSON-encoded literal (a valid Python string literal) sidesteps all
// shell quoting, and pid/timestamps arrive as argv values.
