...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // arbitrary command/path bytes — quotes, newlines, `$$` — cannot corrupt the
    // JSON. The previous `printf '%s' … | sed` approach produced invalid state
    // files for commands containing shell-special characters.
...
function shellQuote(value: string): string {
  // POSIX single-quote escaping: a literal `'` is written as `'\''` — close the
  // quote, emit a double-quoted quote, reopen. The sequence is `'` + `"'"` + `'`
  // = `'"'"'`. (The previous `"'"'"'` started with `"`, which left the opening
  // quote unclosed and corrupted every command/path containing a single quote.)
