...
export class BackgroundTaskExecutionService {
  ...
  async readState(task: BackgroundTaskRecord): Promise<BackgroundTaskExecutionState | undefined> {
    try {
      ...
      // The state file is owned by the detached launch process, which rewrites
      // it on start and on completion. A reader can momentarily observe a
      // mid-write (truncated) file. Treat an unparseable state file as "no
      // readable state yet" rather than letting a single corrupt read throw and
      // abort recovery for every sibling task in a batch.
