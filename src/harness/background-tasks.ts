...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base state payload with pid/timestamps filled in by the python writer below.
  // We deliberately avoid `printf | sed` string-munging here: the substitution is
  // brittle across `sed` implementations (the `"$$"`→pid replace silently no-ops
  // on some, and a command containing quotes corrupts the JSON), which then makes
  // the completed/failed python writer's `json.loads` crash and strand a broken
  // state file. Writing via python's `json.dumps` guarantees valid JSON.
