...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial state atomically (temp + mv) so a concurrent reader
    // never observes a torn/partial JSON file mid-write.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: temp file then os.replace, so readers never see a torn file.
    "import os",
