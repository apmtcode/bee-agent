...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The pid and timestamps are placeholders here (0 / empty); the Python writer
  // below stamps the real values. Writing the payload verbatim via `printf` and
  // mutating it with Python's json module avoids fragile shell/sed escaping —
  // an earlier `sed "s/\"$$\"/$$/g"` form silently left `"pid":"$$"` in place
  // (its backslashes were consumed by the template literal, not bash), producing
...
function shellQuote(value: string): string {
  // POSIX-safe single-quote escaping: close the quote, emit an escaped single
  // quote (\'), then reopen — i.e. each `'` becomes `'\''`. The previous form
  // (`"'"'"'`) started with a `"`, injecting a stray double quote into the
  // result and corrupting any command/path containing a single quote.
