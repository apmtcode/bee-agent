...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The static portion of the initial "running" state. `pid`/`startedAt`/
  // `updatedAt` are filled in at launch time by the Python writer below —
  // building JSON with `printf | sed` is unsafe once the command contains
  // quotes, newlines, or regex metacharacters, and silently corrupted the
  // file (unescaped quotes, unsubstituted `"$$"`). Passing the payload as an
  // argv-quoted string to `python3 json.loads` is robust for any command.
...
function shellQuote(value: string): string {
  ...
  // POSIX idiom: close the quote, emit an escaped quote, reopen (`'\''`). The
  // previous escape (`"'"'"'`) was malformed and corrupted any value containing
  // an apostrophe (e.g. `printf 'x'`), producing invalid shell/JSON downstream.
