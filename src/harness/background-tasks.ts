...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The initial "running" state is written via Python json.dumps (like the
  // completed/failed writers below) rather than shell string-substitution, so
  // commands/cwds containing quotes or newlines can never corrupt the JSON.
  const quotedRunningArgs = [
    quotedStatePath,
    "$$",
    `"$started_at"`,
    shellQuote(task.execution.outputFile),
    quotedCwd,
    quotedCommand,
    shellQuote(task.id),
