...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  const quotedStateTmpPath = shellQuote(`${task.execution.stateFile}.tmp`);
  ...
  return [
    ...
    // Render the initial "running" state, substituting the timestamp and the real
    // shell PID for their placeholders. The sed program is assembled from
    // single-quoted literals concatenated with double-quoted expansions so the
    // literal double-quotes around the pid placeholder survive into the pattern
    // (a single double-quoted sed argument would let them terminate the quote and
    // silently no-op, leaving pid as the string "__OPENCLAW_PID__"). Write to a
    // temp file and atomically rename so concurrent reconcile reads never observe
...
function shellQuote(value: string): string {
  // POSIX-safe single-quoting: end the quote, emit an escaped single quote
  // (`'"'"'`), then reopen the quote. The leading character of the replacement
  // must be a single quote — a malformed escape silently corrupts any command
  // or path containing a single quote.
