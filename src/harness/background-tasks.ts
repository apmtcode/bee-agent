...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The static portion of the running-state payload. pid/startedAt/updatedAt are
  // filled in by the Python writer at launch time. We pass this as a single
  // shell-quoted argv argument (never through sed/printf string substitution),
  // so commands containing quotes, backslashes, or `$$` can never corrupt the
  // emitted JSON — the previous printf|sed approach produced invalid JSON for
  // any command with a single quote.
  ...
  return [
    ...
    // Write the running-state file via json.dumps and an atomic os.replace so a
    // concurrent reader (recovery) never observes a truncated or malformed file.
...
function renderRunningStateWriterPython(): string[] {
  return [
    ...
    // Atomic write: render to a per-pid temp file then os.replace() (POSIX rename)
    // so recovery readers never see a partially written running state.
    "tmp_path = state_path.with_name(state_path.name + '.tmp.' + str(pid))",
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: render to a per-pid temp file then os.replace() (POSIX rename)
    // so recovery readers never see a partially rewritten completion state.
...
function shellQuote(value: string): string {
  ...
  // canonical `'\''` idiom, expressed here as `'"'"'` (close-quote, a
  // double-quoted single quote, reopen-quote). A previous version used
  // `"'"'"'`, which has a spurious leading `"` and rewrote `a'b` into `a"'b`,
  // corrupting any command/path containing a single quote into invalid JSON.
