...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base state written as JSON and passed to python via argv; python parses it
  // and patches pid + timestamps. This avoids `printf | sed` JSON templating,
  // which corrupts payloads whose fields contain quotes or newlines (e.g. the
  // task command) and whose `$`-escapes depend on the local sed dialect.
