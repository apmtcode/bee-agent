...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the base payload verbatim (safely single-quoted) then let python inject
    // the real pid ($$) and timestamp. The previous `sed "s/\"$$\"/$$/g"` never
    // substituted because `$$` expands inside the double-quoted sed script, so the
    // pattern became "<pid>" and never matched the literal "$$" in the JSON — every
    // launched task's initial state therefore carried a non-numeric pid.
