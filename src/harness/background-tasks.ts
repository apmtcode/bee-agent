...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  const quotedStateOutputFile = shellQuote(task.execution.outputFile);
  ...
  return [
    ...
    // Build the initial "running" state with python's json.dumps so arbitrary
    // command/cwd/id content (quotes, newlines, `$$`) can never corrupt the
    // JSON. Values arrive as shell-quoted argv; `$$` is the launcher's real PID.
    `python3 - ${quotedStatePath} $$ "$started_at" ${quotedTaskId} ${quotedKind} ${quotedStateOutputFile} ${quotedCwd} ${quotedCommand} <<'PY'`,
...
function shellQuote(value: string): string {
  ...
  // literal single quote, reopen the quote — i.e. each `'` becomes `'"'"'`.
