...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial "running" state with Python's json module. Each field is
    // passed as a separate shell-quoted argv value (never as pre-serialized JSON)
    // so json.dumps performs all JSON escaping — commands containing quotes,
    // backslashes, or newlines can no longer corrupt the state file. The previous
    // printf+sed writer both mangled such commands and left a literal "$$" pid
    // (its `sed "s/\"$$\"/.../"` used an unescaped `$` end-of-line anchor).
...
function shellQuote(value: string): string {
  // POSIX single-quote escaping: close the quote, emit an escaped quote as
  // `'"'"'` (end-quote, a double-quoted single quote, reopen-quote), then
  // reopen. The previous replacement `"'"'"'` had the double/single quotes
  // transposed, so any value containing a single quote (e.g. a command like
  // `printf 'x'`) reconstructed into a different string in bash — mis-running
  // the command and corrupting the launch-script state file.
