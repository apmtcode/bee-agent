...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The pid and startedAt fields are injected by Python at runtime, so the
  // embedded payload uses inert placeholders. Building the initial state with
  // Python (json.loads of a single-quoted argv) — instead of a printf | sed
  // pipeline — keeps the JSON valid regardless of what quoting the command or
  // cwd contain.
...
function shellQuote(value: string): string {
  ...
  // (\'), then reopen. The replacement string must START with the closing quote
  // — an earlier version used `"'"'"'`, which begins with a double quote and so
  // injected a spurious `"` (and reordered adjacent quotes), corrupting any
  // command or path containing a single quote.
