...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial "running" state via python/json rather than a
    // printf|sed pipeline: shell text substitution corrupts commands that
    // contain quotes or newlines, producing invalid JSON that later crashes
    // recovery. python's json.dumps escapes every value safely.
...
function renderInitialStateWriterPython(task: BackgroundTaskRecord): string[] {
  // JSON.stringify output is a valid python literal for strings/ints, so the
  // task's command/cwd/etc. are embedded safely regardless of quotes/newlines.
  return [
    "import json",
    "import pathlib",
    "import sys",
