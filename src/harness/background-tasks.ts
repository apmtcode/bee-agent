...
export function shellQuote(value: string): string {
  // POSIX single-quote escaping: end the quote, emit an escaped quote in
  // double quotes, then reopen — `'` becomes `'"'"'` (5 chars). The previous
  // sequence started with `"` instead of `'`, corrupting any value that
  // contained a single quote (e.g. a command like `printf 'x'`), which in turn
  // produced invalid JSON in the launch script's state file.
