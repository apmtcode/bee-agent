...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Static running-state fields; the runtime pid/startedAt are injected by the
  // python writer below from argv, so the JSON never carries shell placeholders.
  ...
  return [
    ...
    // Write the initial running state via python (matching the completed/failed
    // writers) instead of a fragile printf|sed substitution — sed could not be
    // trusted to rewrite `"$$"`/timestamps once the command payload contained
    // quotes, which corrupted the JSON.
