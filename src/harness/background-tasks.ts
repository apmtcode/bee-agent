...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The initial "running" state is written by python3 from a base64-encoded JSON
  // payload rather than a printf|sed pipeline. base64 is shell-safe, so commands
  // containing quotes or newlines can no longer corrupt the emitted state.json.
  const initialStateBase64 = encodeStatePayloadBase64({
    version: 1,
    taskId: task.id,
    kind: task.kind,
    status: "running",
  ...
  return [
    ...
    `python3 - ${quotedStatePath} $$ "$started_at" '${initialStateBase64}' <<'PY'`,
...
function encodeStatePayloadBase64(payload: unknown): string {
