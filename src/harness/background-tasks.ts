...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Encode the initial running-state payload as base64 so the launch script can
  // reconstitute it without any shell/sed quoting — commands containing quotes,
  // newlines, or `$` previously corrupted the emitted JSON.
...
function renderRunningStateWriterPython(): string[] {
  return [
    ...
    "state = json.loads(base64.b64decode(sys.argv[4]).decode())",
    "state['pid'] = pid",
