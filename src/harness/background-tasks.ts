...
/**
 * A deterministic {@link SpawnBackgroundProcess} that allocates monotonic fake
 * PIDs and executes nothing. Use it (together with an injected
 * {@link IsProcessRunning}) to drive the background-task subsystem in tests,
 * simulations, or any cloud environment where launching a real detached
 * `run.sh` would be non-deterministic (the real process writes its state file
 * asynchronously, racing state reads) or simply unavailable. Because no process
 * is launched, the on-disk execution state is written only through explicit
 * `writeState` calls, making recovery/derivation behaviour fully deterministic.
 */
export function createSimulatedBackgroundSpawn(startPid = 424_242): SpawnBackgroundProcess {
...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The initial "running" state is written through the same python/json path as
  // the completion writers below. An earlier `printf … | sed` pipeline mangled
  // commands containing quotes or newlines into invalid JSON (the `$$`/timestamp
  // sed substitutions did not survive such payloads), which then crashed every
  // subsequent readState/reconcile. The base payload is base64-encoded so it
  // travels through the shell as a single opaque token with zero quoting
  // hazards; python decodes it and injects the pid + timestamps. This is robust
  // to any command content — quotes, newlines, `$`, backslashes, and all.
  ...
  return [
    ...
    `python3 - ${quotedStatePath} $$ "$started_at" '${encodedStatePayload}' <<'PY'`,
