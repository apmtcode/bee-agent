...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Carry the initial-state payload as base64 so no field value (command,
  // cwd, …) can break shell/JSON quoting, and so the real pid ($$) is resolved
  // by python rather than a fragile sed substitution.
  const initialStatePayloadBase64 = Buffer.from(
    JSON.stringify({
      version: 1,
      taskId: task.id,
      kind: task.kind,
  ...
  return [
    ...
    ...renderInitialStateWriterPython(initialStatePayloadBase64),
    "PY",
...
function renderInitialStateWriterPython(payloadBase64: string): string[] {
  return [
    "import base64",
    "import json",
    "import pathlib",
