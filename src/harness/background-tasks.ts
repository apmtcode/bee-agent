...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The payload is plain JSON (no shell placeholders). `startedAt`/`updatedAt`
  // and `pid` are injected by python at runtime, which keeps the JSON valid for
  // arbitrary command strings — sed-based substitution corrupted the file
  // whenever a field contained quotes, `$`, or newlines.
  ...
  return [
    ...
    // Write the initial running state via python (robust JSON) and atomically
    // (temp file + os.replace) so a concurrent reader in the managing process
    // never observes a half-written or sed-mangled, unparseable file.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic replace so the managing process never reads a torn state file.
    "tmp_path = state_path.with_name(state_path.name + '.' + str(pid) + '.tmp')",
    "tmp_path.write_text(json.dumps(state, indent=2) + '\\n')",
    "os.replace(tmp_path, state_path)",
  ];
...
function shellQuote(value: string): string {
  ...
  // reopen. The previous `"'"'"'` replacement started with a double quote and
  // left a stray `"` in the output, corrupting any value (e.g. a command) that
