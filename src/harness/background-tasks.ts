...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // command/cwd strings (quotes, backslashes, newlines) can't corrupt the
    // document — and atomically (temp file + os.replace) so a concurrent reader
    // never observes a truncated or half-written state file.
...
function renderRunningStateWriterPython(): string[] {
  return [
    ...
    "    'command': sys.argv[6],",
    "}",
    // Atomic write: serialize to a sibling temp file then os.replace() so
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: serialize to a sibling temp file then os.replace() (an
    // atomic rename) so readers only ever see a complete JSON document.
    "tmp_path = state_path.with_name(state_path.name + '.tmp.' + str(os.getpid()))",
    "tmp_path.write_text(json.dumps(state, indent=2) + '\\n')",
    "os.replace(tmp_path, state_path)",
  ];
