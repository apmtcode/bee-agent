...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Pass every dynamic value to python3 as an argv element (bash-quoted here),
  // so python — not a fragile printf/sed round-trip — serializes the JSON. This
  // is robust to commands containing quotes, newlines, backslashes, etc., which
  // previously corrupted the initial state file. State is written atomically
  // (temp file + rename) so a concurrent reconciler never sees a partial file.
...
function shellQuote(value: string): string {
  ...
  // sequence '\'' — close the quote, emit an escaped literal quote, reopen.
  // (The previous sequence "'"'"' was malformed and left an unbalanced double
  // quote, corrupting any command that contained a single quote.)
