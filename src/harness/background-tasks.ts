...
export class BackgroundTaskExecutionService {
  ...
  async writeArtifacts(task: BackgroundTaskRecord): Promise<void> {
    ...
    // Persist the base "running" state as a correctly-encoded JSON template so
    // the launch script never has to shell-quote/`sed` the command into JSON
    // (which corrupted state for any command containing quotes/backslashes).
    // The launch script fills in pid + timestamps and writes state atomically.
    const templatePath = path.join(this.rootDir, stateTemplateFile(task));
    await ensureParentDir(templatePath);
    await fs.writeFile(
      templatePath,
      `${JSON.stringify(buildRunningStateTemplate(task), null, 2)}\n`,
...
function stateTemplateFile(task: BackgroundTaskRecord): string {
  return `${task.execution.stateFile}.tmpl`;
}

function buildRunningStateTemplate(task: BackgroundTaskRecord): Omit<BackgroundTaskExecutionState, "pid" | "startedAt" | "updatedAt"> {
  return {
    version: 1,
    taskId: task.id,
    kind: task.kind,
...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  const quotedTemplatePath = shellQuote(stateTemplateFile(task));
  ...
  return [
    ...
    // Fill pid + timestamps into the Node-encoded running-state template and
    // write it atomically. No shell-quoting of the command into JSON, so
    // commands containing quotes/backslashes can no longer corrupt the state.
    `python3 - ${quotedTemplatePath} ${quotedStatePath} $$ "$started_at" <<'PY'`,
...
// Shared atomic-write tail: serialize `state` to a pid-unique temp file then
// os.replace() over the real path so readers see either the old or new complete
// document, never a truncated one mid-write.
const PYTHON_ATOMIC_WRITE_TAIL = [
...
function shellQuote(value: string): string {
  ...
  // close the quote, emit a quoted literal quote, reopen — i.e. ' -> '"'"'.
