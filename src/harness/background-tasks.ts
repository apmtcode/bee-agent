...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Task-defined fields (id, kind, command, cwd, outputFile) can contain quotes,
  // backslashes, and shell metacharacters. They are passed to Python as argv and
  // serialized with json.dumps so JSON escaping is always correct — never built by
  // string interpolation or sed munging on the shell side.
