...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Build the initial "running" state in Python (already required below) rather
    // than assembling JSON in bash with printf+sed. Every dynamic value is passed
    // via argv, so a command containing quotes, `$`, or backslashes can never
    // corrupt the document — and the write is atomic (temp sibling + os.replace),
    // so a concurrent reconcile reader never observes a partial file.
    `python3 - ${quotedStatePath} ${quotedTaskId} ${quotedKind} $$ "$started_at" ${quotedOutputFile} ${quotedCwd} ${quotedCommand} <<'PY'`,
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Write atomically: a temp sibling then os.replace, so reconcile readers see
