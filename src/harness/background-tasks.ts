...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial state via python (already required below) rather than a
    // printf | sed pipeline: passing the JSON payload as a single-quoted argv
    // preserves arbitrary command strings (quotes, newlines) that would corrupt
    // sed-substituted output and yield unparseable state.json on recovery.
