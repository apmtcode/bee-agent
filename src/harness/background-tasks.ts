...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base payload carries only the static fields. pid + timestamps are injected at
  // runtime by the Python writer below, so no fragile shell/sed substitution is
  // needed and arbitrary command text (quotes, backslashes, newlines) is safe:
  // the whole payload rides through as a single shell-quoted JSON argument that
  // Python parses back into an object.
  ...
  return [
    ...
    // Write the initial "running" state atomically and safely via Python: parse the
    // base JSON, inject pid + timestamps, stage to a temp file, then os.replace().
    // A concurrent reader (recovery/sync) sees either the old file or the whole new
    // one — never a torn or malformed write, regardless of command content.
...
function renderInitialStateWriterPython(): string[] {
  return [
    ...
    // Atomic initial write: stage to a temp file then os.replace() (atomic on
    // POSIX + Windows) so a concurrent reader never observes a truncated file.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic terminal write: stage to a temp file then os.replace() (atomic on
    // POSIX + Windows) so a concurrent reader never observes a truncated file.
    "tmp_path = state_path.with_name(state_path.name + f'.{os.getpid()}.tmp')",
    "tmp_path.write_text(json.dumps(state, indent=2) + '\\n')",
    "os.replace(tmp_path, state_path)",
...
function shellQuote(value: string): string {
  ...
  // idiom '"'"' (close quote, a double-quoted literal quote, reopen quote). The
  // replacement must be exactly 5 chars; a stray leading double-quote corrupts
  // every value that contains a single quote (commands, cwd paths, JSON payloads).
