...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  const quotedStatePayload = shellQuote(
    JSON.stringify({
      ...
      kind: task.kind,
      status: "running",
      // Placeholder — substituted with the launched shell's numeric pid ($$) by
      // the sed pipeline below. Must be a *string* here so JSON.stringify emits
      // quotes we can strip during substitution, yielding a numeric JSON pid.
  ...
  return [
    ...
    // Substitute the timestamp and pid placeholders, then write the initial
    // state atomically (temp file + mv) so a concurrent reader never observes a
    // partially-written, unparseable JSON document. Each sed script is
    // single-quoted so the literal `"` around the pid placeholder can never
    // break the shell quoting (the previous double-quoted form expanded `$$`
    // wrongly and left `"pid":"$$"` unsubstituted, so every task's state carried
    // a non-numeric pid and was misreported as a missing process).
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic replace: write to a sibling temp file, then os.replace() so readers
    // only ever see a complete JSON document (never a truncated in-progress write).
...
function shellQuote(value: string): string {
  // Wrap in single quotes and escape any embedded single quote via the
  // canonical POSIX sequence `'"'"'` (close-quote, "'", reopen-quote). The
  // replacement MUST NOT start with a `"` — a leading double quote injects a
  // stray `"` into the output, which previously corrupted the JSON state file
  // (and any launched command) whenever the command contained a single quote.
