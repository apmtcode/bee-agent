...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  const quotedStatePayload = shellQuote(
    JSON.stringify({
      ...
      kind: task.kind,
      status: "running",
      // Placeholders substituted below. `pid` is written WITH quotes here and
      // the sed pattern strips them, so the emitted value is a bare number.
  ...
  return [
    ...
    // observes a half-written state.json (torn read -> JSON parse error). The
    // pid replacement uses a single-quoted sed expression so the literal double
    // quotes around the placeholder survive into the shell verbatim.
    `printf '%s' ${quotedStatePayload} | sed -e "s|__OPENCLAW_STARTED_AT__|$started_at|g" -e 's|"__OPENCLAW_PID__"|'"$$"'|g' > ${quotedStateTmpPath} && mv -f ${quotedStateTmpPath} ${quotedStatePath}`,
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: temp file + os.replace, so readers never see a torn state.
...
function shellQuote(value: string): string {
  // POSIX single-quote escape: end the quote, emit a quoted apostrophe, reopen.
