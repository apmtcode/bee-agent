...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base state payload; pid + timestamps are injected by Python at launch time
  // (via json.dumps) so that commands containing quotes/newlines cannot corrupt
  // the JSON — the earlier `printf | sed` substitution both failed to replace the
  // "$$" pid placeholder and produced invalid JSON for such commands.
