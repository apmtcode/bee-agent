...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Encode the initial "running" state as base64 so it can be embedded in the
  // launch script without any shell/JSON quoting hazards. The previous
  // `printf | sed` approach corrupted the payload for commands containing
  // single quotes or newlines, producing invalid JSON that then crashed the
  // Python completed/failed writers (they json.loads() this file first).
  const runningStatePayloadB64 = Buffer.from(
    JSON.stringify({
      version: 1,
      taskId: task.id,
      kind: task.kind,
  ...
  return [
    ...
    ...renderRunningStateWriterPython(runningStatePayloadB64),
    "PY",
...
function renderRunningStateWriterPython(payloadBase64: string): string[] {
  return [
    "import base64",
    "import json",
    "import pathlib",
    ...
    `state = json.loads(base64.b64decode('${payloadBase64}').decode('utf-8'))`,
    "state['pid'] = pid",
...
function shellQuote(value: string): string {
  ...
  // the quoted run, emitting an escaped quote, then reopening it — `'\''`.
  // (The previous `"'"'"'` form injected a spurious double quote, corrupting
  // any command that contained a single quote, e.g. `printf 'x'`.)
