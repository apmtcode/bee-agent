...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The dynamic pid/started_at are injected by python at launch time; keeping
  // them out of the template avoids fragile shell/sed placeholder substitution.
  ...
  return [
    ...
    // Write the initial "running" state via python (payload passed verbatim as an
    // argv, not round-tripped through printf|sed which mangled shell-quoted
    // commands) and rename atomically so a concurrent reader never sees partial JSON.
