...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Initial "running" state is written by python from argv-passed values rather
    // than by hand-building JSON in the shell. Embedding the JSON payload in a
    // shell string and rewriting it with sed corrupted commands containing
    // quotes/newlines and left the "$$" pid placeholder unsubstituted.
...
// Atomically replace the state file so a concurrent reader (recovery/reconcile)
