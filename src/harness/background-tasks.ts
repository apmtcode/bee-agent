...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial "running" state via python (with all dynamic fields
    // passed as literal argv) rather than reconstructing JSON with printf|sed.
    // The old sed-based approach broke on any command containing quotes and
    // silently failed to substitute the pid/timestamp, leaving state.json
    // malformed until the process exited.
...
function shellQuote(value: string): string {
  // POSIX-safe single-quoting: to embed a literal single quote inside a
  // single-quoted string, close the quote, emit a double-quoted quote, then
  // reopen — i.e. replace `'` with `'"'"'`. (The previous form had a spurious
  // leading double quote, `"'"'"'`, which injected a stray `"` and corrupted
  // any command containing single quotes.)
