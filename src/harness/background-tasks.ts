...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial state via python's json (never hand-rolled sed
    // substitution): task.command can contain shell metacharacters (quotes,
    // newlines) that corrupted the JSON when injected through printf|sed. The
    // base payload is passed as a single argv value, and started_at/pid are
    // filled in and the file replaced atomically.
...
function renderInitialStateWriterPython(): string[] {
  return [
    ...
    "state['startedAt'] = sys.argv[2]",
    "state['updatedAt'] = sys.argv[2]",
