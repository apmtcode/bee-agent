...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The base payload is JSON.stringify'd here (correct escaping for any command)
  // and passed verbatim as a single shell argument. `pid`/`startedAt`/`updatedAt`
  // are placeholders that the initializer overwrites with real runtime values.
  // We intentionally do NOT build the state file via `printf | sed`: sed treats
  // `$` as a regex anchor (so `"$$"` never matched, leaving pid a string) and
  // mangled the JSON escaping of commands containing quotes/newlines, producing
  // invalid JSON that broke recovery. Delegating to python3 (already required
  // for the terminal-state writers) keeps the file valid for any command.
...
function shellQuote(value: string): string {
  ...
  // the quoted section, emitting an escaped quote, and reopening — `'\''`.
  // (The previous sequence `"'"'"'` was mis-ordered and injected a spurious `"`
  // into any value containing a single quote, corrupting both the executed
  // command and the JSON state payload.)
