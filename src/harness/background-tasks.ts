...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial state atomically (temp file + mv) so a concurrent reader
    // never observes a truncated/torn JSON document mid-write.
    //
    // The pid substitution replaces the literal token `"$$"` in the JSON payload
    // with the launcher's numeric pid. The double quotes around `$$` must reach
    // sed as *literal* characters, so they are escaped (\") — otherwise they
    // close/reopen bash's double-quoted sed argument, the pattern collapses to
    // `s/<pid>/<pid>/g`, and `"pid"` is left as the string "$$" (which then fails
    // every isProcessRunning() check and mis-reports tasks as missing-process).
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: render to a temp file then os.replace() so readers only ever
    // see a complete state document (matches writeJsonAtomic on the TS side).
...
function shellQuote(value: string): string {
  // Wrap in single quotes and escape any embedded single quote using the POSIX
  // idiom: close the quote, emit a double-quoted single quote, then reopen.
  // The replacement is exactly '"'"' — a leading double quote (as in "'"'"')
  // leaks a literal `"` into the output and corrupts values such as
  // `printf 'a\nb'`, producing invalid JSON in the task state file.
