...
export class BackgroundTaskExecutionService {
  ...
  async readState(task: BackgroundTaskRecord): Promise<BackgroundTaskExecutionState | undefined> {
    try {
      ...
      // A state file that is mid-write, truncated, or corrupted must not crash
      // a recovery sweep over sibling tasks. Treat unreadable state as absent so
      // the caller degrades to the missing-state reconciliation path instead.
...
// Writes the initial "running" state. The real integer pid arrives as argv[2]
// ($$ of the launch script, which is the detached process-group leader), so
// isProcessRunning() can later check the group. The write goes to a temp file
// and is atomically renamed so a concurrent recovery read never sees torn JSON.
