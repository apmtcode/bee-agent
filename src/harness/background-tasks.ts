...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial "running" state via python3 (already required by the
    // completion path below) rather than embedding JSON in the shell and
    // rewriting it with sed. Passing every field as a shell-quoted argv element
    // avoids fragile in-shell JSON escaping — the old printf|sed approach
    // mangled commands containing quotes/newlines and never substituted the pid
    // placeholder, leaving `pid` as the literal string "$$". python writes to a
    // temp file and atomically renames, so concurrent readers never observe a
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: render to a temp file in the same directory, then rename over
    // the target so readers never observe a partially written state file.
    "tmp_path = state_path.with_name(state_path.name + f'.{pid}.tmp')",
    "tmp_path.write_text(json.dumps(state, indent=2) + '\\n')",
    "tmp_path.replace(state_path)",
  ];
...
function shellQuote(value: string): string {
  ...
  // canonical POSIX sequence (close-quote, escaped literal quote, reopen-quote).
  // The previous replacement ("'"'"') was malformed and corrupted any command
  // containing a single quote, e.g. `printf 'hi'` round-tripped to `printf "'hi"'`.
