...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The static portion of the running-state payload (no pid/timestamps). Passed
  // to python as a single shell-quoted argv and parsed with json.loads, so the
  // command/cwd values cannot corrupt the JSON regardless of the characters they
  // contain. Avoids the previous fragile `printf | sed` munging which mangled
  // quotes in `command` and left `pid` as the literal "$$" (since sed treats `$`
  // as an anchor).
...
function shellQuote(value: string): string {
  // POSIX-safe single-quote escaping. Wrap the value in single quotes and
  // replace each embedded single quote with the 5-char sequence '"'"' — close
  // the quote, emit a literal quote via "'", then reopen. The previous sequence
  // ("'"'"', 6 chars, leading double-quote) mis-escaped, turning a `'` into `"'`
  // and corrupting both the JSON state payload and the command passed to
  // `bash -lc` for any command containing a single quote.
