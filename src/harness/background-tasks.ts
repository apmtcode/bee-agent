...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial running state via python read-modify-write (same robust
    // path as the completed/failed writers below). The previous `printf | sed`
    // approach broke bash double-quoting on `"$$"`, leaving pid as the literal
    // string "$$" instead of the real process id.
...
function shellQuote(value: string): string {
  // POSIX-safe single-quote escaping: end the quote, emit a quoted literal
  // single quote ('"'"'), then reopen. The previous sequence ("'"'"') placed
  // the quotes in the wrong order, corrupting any command that contained a
  // single quote (mangled state JSON, unsubstituted "$$" pid). See runner.ts.
