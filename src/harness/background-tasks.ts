...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  const quotedStartupMessage = shellQuote(`starting ${task.kind} ${task.id}`);

  // The running/completed/failed state files are all emitted by python3
  // reading their variable fields from argv (each shell-quoted as a single
  // word) and serialising with json.dumps + an atomic temp-file replace. This
  // is robust against arbitrary characters in the command/cwd (quotes,
  // newlines, backslashes) and avoids torn reads — a previous printf+sed
  // approach silently corrupted the JSON for commands containing quotes and
  // never substituted the pid (recording the literal string "$$"), so every
  // launched task falsely looked like a "missing-process".
  return [
    "#!/usr/bin/env bash",
    "set -euo pipefail",
    ...
    "PY",
    `printf '%s\n' ${quotedStartupMessage} >> ${quotedOutputFile}`,
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write (temp + os.replace) so a concurrent reader never observes a
    // torn/partial JSON file.
    "tmp_path = state_path.with_name(state_path.name + f'.{pid}.tmp')",
    "tmp_path.write_text(json.dumps(state, indent=2) + '\\n')",
    "os.replace(tmp_path, state_path)",
  ];
...
function shellQuote(value: string): string {
  // Wrap in single quotes and escape embedded single quotes with the canonical
  // POSIX sequence `'"'"'` (close-quote, a double-quoted single quote, reopen).
  // A previous version used `"'"'"'`, which reconstructs `a'b` as `a"'b` — i.e.
  // it silently corrupted every command containing a single quote, both when
  // executing it (`bash -lc`) and when recording it to the state file.
