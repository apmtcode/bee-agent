...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // The initial "running" state is written by python3 (the same dependency the
    // completion writers below use) with every dynamic field passed as a
    // separate argv element. This avoids brittle `printf | sed` JSON templating,
    // which silently produced invalid state.json whenever the command contained
    // quotes/newlines and never substituted the pid placeholder.
...
function shellQuote(value: string): string {
  ...
  // replacement (`"'"'"'`) had the quote order wrong and turned `a'b` into
  // `a"'b`, corrupting any command/path containing a single quote.
