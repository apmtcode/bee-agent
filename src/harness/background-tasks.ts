...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial state atomically: render to a pid-scoped temp file and
    // rename into place, so a concurrent reader (recovery/sync) never observes a
    // torn, half-written state.json.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: render to a temp sibling then os.replace() into place so a
    // concurrent reader never sees a partially written state.json.
...
function shellQuote(value: string): string {
  // POSIX single-quote escaping: a literal ' is emitted by closing the quote,
  // adding a double-quoted quote, then reopening — `'"'"'`. The sequence must
  // start by closing the surrounding quote; a leading `"` (the old `"'"'"'`)
  // leaves the quote open and mangles any value containing a single quote.
