...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial state to a temp file then rename it into place so a
    // concurrent reader never observes a torn (mid-write) state.json.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: serialize to a sibling temp file, then os.replace() it over
    // the target so a concurrent reader sees either the old or the new state,
