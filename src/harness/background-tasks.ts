...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the state file atomically: emit to a unique temp path, then rename
    // into place. A plain `> file` redirect truncates then writes, so a
    // concurrent reader can observe a half-written (unparseable) state file.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // place so a concurrent reader never sees a truncated state file.
...
function shellQuote(value: string): string {
  // POSIX-safe single-quote escaping: end the quote, emit an escaped quote via a
  // double-quoted section, then reopen. A single literal `'` becomes `'\''` in
  // the canonical `'"'"'` form. The previous form (`"'"'"'`) had a stray leading
  // double quote, so any value containing `'` (e.g. a shell command that quotes
  // its argument) produced a corrupt token — the launched state writer then
  // emitted unparseable JSON with an unescaped `"` in the command field.
