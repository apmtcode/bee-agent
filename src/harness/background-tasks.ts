...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The running-state payload is written by python (json.loads/json.dumps), not
  // shell string munging: the previous `printf | sed` approach corrupted any
  // command containing single quotes and never substituted the pid (sed treats
  // `$` in `"$$"` as an end-of-line anchor, so the literal `"$$"` string leaked
  // into the state file). python receives the payload as a safely-quoted argv
  // and injects the real pid + started_at.
...
function shellQuote(value: string): string {
  ...
  // The canonical sequence for an embedded `'` is `'\''` (equivalently `'"'"'`).
