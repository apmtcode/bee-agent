...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base payload without pid/startedAt — Python injects those so the JSON is
  // always valid, even when the command contains quotes, backslashes, or
  // newlines. (The previous `printf | sed` approach mangled such commands and
  // failed to substitute the `$$`→pid placeholder on non-GNU sed.)
...
function shellQuote(value: string): string {
  // Escape single quotes for a single-quoted shell word: close the quote, emit a
  // double-quoted literal quote, reopen. The correct sequence is '"'"' — the
  // previous "'"'"' inserted a spurious leading double quote, corrupting any
  // command that itself contained single quotes (e.g. `printf '%s' '…'`).
