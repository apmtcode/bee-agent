...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The base state carries every static field. The runtime-only values (pid,
  // timestamps) are injected by Python at launch — see renderInitialStateWriter.
  const quotedBasePayload = shellQuote(
    JSON.stringify({
      version: 1,
      taskId: task.id,
      kind: task.kind,
  ...
  return [
    ...
    // Write the initial running-state via Python so the base JSON (which may
    // contain quotes, newlines or shell/sed metacharacters in the command) is
    // decoded and re-serialised safely, then swapped in atomically. The old
    // `printf | sed` approach corrupted the file for such commands.
    `OPENCLAW_STATE_BASE=${quotedBasePayload} OPENCLAW_STATE_PATH=${quotedStatePath} OPENCLAW_STATE_PID=$$ OPENCLAW_STATE_TS="$started_at" python3 - <<'PY'`,
...
function renderInitialStateWriterPython(): string[] {
  return [
    ...
    "state['pid'] = int(os.environ['OPENCLAW_STATE_PID'])",
    "state['startedAt'] = os.environ['OPENCLAW_STATE_TS']",
    "state['updatedAt'] = os.environ['OPENCLAW_STATE_TS']",
    "state_path = pathlib.Path(os.environ['OPENCLAW_STATE_PATH'])",
    // Atomic replace so a concurrent readState() never sees a partial write.
