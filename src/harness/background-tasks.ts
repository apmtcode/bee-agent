...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The dynamic pid ($$) and started_at timestamp are injected by python at
  // runtime; everything else is a static snapshot serialized here. Passing the
  // JSON payload to python (json.loads) rather than post-processing it with
  // `sed` keeps arbitrary command text (quotes, newlines, backslashes, `&`)
  // from corrupting the resulting state file.
...
function shellQuote(value: string): string {
  // POSIX single-quote escaping: a literal `'` cannot appear inside a
  // single-quoted string, so close the quote, emit an escaped quote via a
  // double-quoted `'`, and reopen — i.e. replace each `'` with `'"'"'`.
