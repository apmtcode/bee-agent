...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // reader (recovery/sync/status) never observes a partially written file.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic terminal-state write so readers never see a torn file.
...
function shellQuote(value: string): string {
  ...
  // quote (`'"'"'`), reopen. The previous sequence had a stray leading double
  // quote (`"'"'"'`), which corrupted any command containing a single quote —
  // e.g. `printf 'x'` — mangling both the launch command and the state file.
