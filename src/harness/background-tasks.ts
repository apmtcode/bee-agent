...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial state atomically: render to a per-pid temp file, then
    // rename over the destination. A bare `> stateFile` truncates in place, so a
    // concurrent reader (syncBackgroundTask → readState) can observe a partially
    // written file and fail to parse the JSON.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // (an atomic rename) so a concurrent reader never sees a truncated file.
...
function shellQuote(value: string): string {
  ...
  // canonical POSIX sequence `'\''` (close-quote, escaped quote, re-open). The
  // previous sequence `"'"'"'` did not round-trip, so any command containing a
  // single quote produced a corrupt state file (invalid JSON on read).
