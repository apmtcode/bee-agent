...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Emit the initial "running" state as valid JSON without round-tripping the
  // payload through shell quoting. A quoted heredoc (`<<'DELIM'`) writes the
  // JSON byte-for-byte with zero shell interpretation, so commands containing
  // single quotes, double quotes, or backslashes can't corrupt the file. The
  // runtime-only fields (pid, timestamps) are left null and filled in by the
  // same robust `python3 json` step used for completed/failed states.
  ...
  return [
    ...
    `cat > ${quotedStatePath} <<'OPENCLAW_STATE_JSON'`,
    initialStatePayload,
    "OPENCLAW_STATE_JSON",
