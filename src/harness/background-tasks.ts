...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Base state is serialized in Node (correct JSON escaping for arbitrary
  // command/cwd content) and handed to a python writer that injects the runtime
  // pid + timestamps. Building the JSON inside bash via printf|sed corrupted the
  // file whenever the command contained quotes or newlines.
