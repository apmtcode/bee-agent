...
function shellQuote(value: string): string {
  // POSIX-safe single-quote escaping: close the quote, emit a quoted
  // single-quote (`'"'"'`), then reopen. The leading char MUST be `'`
  // (not `"`) or each embedded quote renders as the two chars `"'`,
  // corrupting payloads/commands that contain single quotes.
