...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The initial state is written by python via json.dumps rather than
  // `printf | sed` so that the pid/timestamp placeholders can be injected
  // without a shell/sed pass mangling the JSON escaping of the payload — a
  // command containing quotes, backslashes, or newlines previously produced an
  // invalid state.json that broke recovery. python receives the static fields
  // as one already-valid JSON argv element and fills pid/startedAt itself.
  const quotedInitialStatePayload = shellQuote(
    JSON.stringify({
      version: 1,
      taskId: task.id,
      kind: task.kind,
  ...
  return [
    ...
    `python3 - ${quotedStatePath} $$ "$started_at" ${quotedInitialStatePayload} <<'PY'`,
