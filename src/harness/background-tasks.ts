...
function shellQuote(value: string): string {
  // POSIX single-quote escaping: end the quoted run, emit a double-quoted
  // apostrophe, then reopen — i.e. each `'` becomes `'"'"'`. A malformed
  // variant (`"'"'"'`) fails to close the opening quote and corrupts any
  // command containing a single quote (e.g. `printf 'x'`), producing invalid
  // JSON in the state file the launch script writes.
