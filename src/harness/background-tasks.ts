...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The initial "running" state is written by python from a base64-encoded JSON
  // template. Base64 is shell-safe (alphanumeric + "+/="), so arbitrary command
  // strings — including ones with single quotes, double quotes, or newlines —
  // can never break the surrounding shell quoting the way a printf|sed pipeline
  // did. `pid`/`startedAt`/`updatedAt` are stamped in python from argv.
...
function renderInitialStateWriterPython(): string[] {
  return [
    ...
    // Atomic write: render to a pid-scoped temp file then os.replace() into place
    // so readers (recover/sync) never see a torn JSON document.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: render to a pid-scoped temp file then os.replace() into place
    // so readers never see a torn JSON document.
    "tmp_path = state_path.with_name(state_path.name + f'.{pid}.tmp')",
    "tmp_path.write_text(json.dumps(state, indent=2) + '\\n')",
    "tmp_path.replace(state_path)",
  ];
