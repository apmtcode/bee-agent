...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  const quotedStatePayload = shellQuote(
    JSON.stringify({
      ...
      kind: task.kind,
      status: "running",
      // Placeholder substituted for the real numeric PID by the launch script.
      // Quoted so it is valid JSON on disk; the sed replacement strips the
      // surrounding quotes, leaving a bare number.
  ...
  return [
    ...
    // Write the running-state atomically (temp file + mv) so concurrent readers
    // never observe a truncated/empty state.json. `__OPENCLAW_PID__` has no regex
    // metacharacters, so unlike the previous `"$$"` pattern (where `$` was read as
    // an end-of-line anchor and never matched) the pid is reliably substituted.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: a reader must never see a half-written state file.
