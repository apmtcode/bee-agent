...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The initial "running" state is written by Python's json module (not a
  // printf|sed pipeline) so that commands containing quotes, `$`, or newlines
  // cannot corrupt the JSON or leave the `$$`/timestamp placeholders unsubstituted.
...
function renderInitialStateWriterPython(): string[] {
  return [
    ...
    // The launch-script's own PID ($$), passed by bash. It stays alive for the
    // duration of the task, so liveness checks against it are meaningful — unlike
    // the transient python writer's own pid.
    "pid = int(sys.argv[3])",
