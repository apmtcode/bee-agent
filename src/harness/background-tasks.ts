...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Build the initial running state in Python from argv values (not by
    // string-substituting a pre-rendered JSON payload). Passing the taskId,
    // kind, cwd, and command as separate argv arguments means the values are
    // never re-parsed by the shell or a `sed` program, so commands containing
    // quotes, backslashes, or `$$` can no longer corrupt the JSON. Python's
    // json module handles all escaping and the writer renames atomically.
...
function renderInitialStateWriterPython(): string[] {
  return [
    ...
    "    'command': sys.argv[8],",
    "}",
    // Atomic write via a sibling temp file + replace, so a concurrent reader
    // (recovery/reconcile) never observes a half-written state file.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // place, so a concurrent reader never observes a half-written state file.
    "tmp_path = state_path.with_name(state_path.name + '.tmp.' + str(pid))",
    "tmp_path.write_text(json.dumps(state, indent=2) + '\\n')",
    "tmp_path.replace(state_path)",
  ];
