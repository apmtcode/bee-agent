...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    `printf '%s' ${quotedStatePayload} | sed "s/__OPENCLAW_STARTED_AT__/$started_at/g; s/\\"\\$\\$\\"/$$/g" > ${quotedStatePath}.launch.tmp`,
    `mv ${quotedStatePath}.launch.tmp ${quotedStatePath}`,
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    "tmp_path = state_path.with_name(state_path.name + f'.state.{pid}.tmp')",
