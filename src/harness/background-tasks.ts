...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write the initial state atomically: substitute the timestamp/pid
    // placeholders (the quoted `"__OPENCLAW_PID__"` becomes an unquoted numeric
    // pid), render to a temp file, then rename over the target so a concurrent
    // reader never observes a truncated or malformed file. The sed quotes are
    // backslash-escaped so they do not prematurely close the shell double-quote.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic write: emit to a temp sibling then os.replace over the target so a
    // concurrent reader never sees a partially-written state file.
...
function shellQuote(value: string): string {
  // POSIX single-quote escaping: to embed a literal `'` inside a single-quoted
  // string, close the quote, emit an escaped quote via `"'"`, then reopen —
  // i.e. replace each `'` with the 5-char sequence `'"'"'`. The previous
  // 6-char `"'"'"'` was malformed: it injected a spurious `"` into the output,
  // corrupting any command/JSON payload that contained a single quote.
