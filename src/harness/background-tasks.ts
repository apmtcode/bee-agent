...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // Write a valid JSON skeleton, then let python patch in the real pid and
    // timestamps. The previous printf|sed approach corrupted the file whenever
    // the command (or any embedded field) contained double quotes, because the
    // sed `"$$"` pattern prematurely closed the shell quote.
