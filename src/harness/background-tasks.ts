...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // — e.g. crash recovery scanning task state — never observes a partial write.
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic replace: write to a sibling temp file then os.replace() (atomic on
    // POSIX + Windows) so recovery never reads a half-serialized state file.
