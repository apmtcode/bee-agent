...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // Initial "running" state payload WITHOUT the runtime-only fields (pid,
  // timestamps) — those are injected by the Python writer from argv. Building
  // the JSON via `json.loads(sys.argv[...])` rather than `printf | sed` keeps
  // the escaping correct for commands containing quotes/backslashes/newlines.
