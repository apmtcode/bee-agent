...
export class BackgroundTaskExecutionService {
  ...
  async readState(task: BackgroundTaskRecord): Promise<BackgroundTaskExecutionState | undefined> {
    ...
    // Defense-in-depth: the launch script now writes state atomically, but if a
    // non-atomic writer (older artifact, external tooling) leaves the file torn
    // mid-write, a bare read would throw a JSON SyntaxError and abort the whole
    // recover/sync/status operation. Retry a couple of times on parse failure —
    // a torn window closes within milliseconds — before surfacing the error.
...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  return [
    ...
    // concurrent reader — remote-status polling, sync, or recovery — can never
    // observe a half-written state file and crash on torn JSON.
    `printf '%s' ${quotedStatePayload} | sed "s/__OPENCLAW_STARTED_AT__/$started_at/g; s/\"\$\$\"/$$/g" > ${quotedStateTmpPath}`,
    `mv -f ${quotedStateTmpPath} ${quotedStatePath}`,
...
function renderStateWriterPython(status: BackgroundTaskExecutionState["status"]): string[] {
  return [
    ...
    // Atomic terminal-state write: stage to a temp file, then rename into place
    // so concurrent readers never see a partially serialized state document.
