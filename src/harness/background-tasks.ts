...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The static portion of the running state is passed to python as base64 so
  // arbitrary command strings (quotes, newlines, backslashes) can never corrupt
  // the emitted JSON. python owns all JSON serialization, matching the
  // completed/failed writers below.
  const seedBase64 = Buffer.from(
    JSON.stringify({
      version: 1,
      taskId: task.id,
      kind: task.kind,
  ...
  return [
    ...
    `python3 - ${quotedStatePath} $$ '${seedBase64}' <<'PY'`,
...
function renderRunningStateWriterPython(): string[] {
  return [
    ...
    "pid = int(sys.argv[2])",
    "state = json.loads(base64.b64decode(sys.argv[3]))",
