...
function renderLaunchScript(task: BackgroundTaskRecord): string {
  ...
  // The initial "running" state is assembled inside Python from individually
  // shell-quoted scalars — never a pre-serialized JSON blob passed as one shell
  // arg. `pid` is written as a real *number* ($$); an earlier `sed` approach left
  // it the literal string "$$" (its embedded quotes closed the shell string),
  // which broke every `isProcessRunning(state.pid)` check and made live tasks
  // look dead. All state writes go through a temp file + os.replace so a
...
// Python that emits `state` to `state_path` atomically (temp file + os.replace)
// so a concurrent reader never observes a half-written file. `> file` truncation
// used previously left a window where `readState` parsed partial JSON and threw.
const PYTHON_ATOMIC_WRITE: string[] = [
