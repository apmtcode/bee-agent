...
describe("BackgroundTaskExecutionService", () => {
  ...
  it("runs the generated launch script end-to-end, writing valid state JSON for a single-quoted command", async () => {
    const rootDir = await makeTempDir();
    const store = new FileBackgroundTaskStore(path.join(rootDir, "background-tasks.json"), () => ({ pid: 4242, unref() {} }));
    // A realistic single-line command containing single quotes — the shape that
    // previously corrupted state.json when the initial "running" state was
    // generated by shell `printf | sed` string interpolation (the single quotes
    // broke the shell-quoted JSON payload, producing invalid JSON that then
    // crashed the completion writer's json.loads). The command must survive
    // verbatim into the persisted state.
    const command = `printf '%s' 'hello world'`;
    const task = await store.start({ title: "Emit lines", command, cwd: rootDir, kind: "task" });

    // The store's spawn is mocked, so run the generated script ourselves.
    const scriptPath = path.join(rootDir, task.execution.launchScript);
    await runScript("/usr/bin/env", ["bash", scriptPath], { cwd: rootDir });
    ...
    expect(state.command).toBe(command); // single quotes round-trip intact
