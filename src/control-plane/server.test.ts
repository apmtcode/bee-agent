...
describe("OperatorControlPlaneServer", () => {
  it("handles session, transcript, approval, trajectory, memory, and orchestration methods", async () => {
    ...
    const runtime = new StandaloneOperatorRuntime({
      rootDir,
      // Stub the spawner so background tasks don't launch real OS processes whose
      // asynchronously-written "running" state would otherwise race with this
      // test's deterministic remote-status expectations.
