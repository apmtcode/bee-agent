...
describe("OperatorControlPlaneServer", () => {
  it("handles session, transcript, approval, trajectory, memory, and orchestration methods", async () => {
    ...
    const runtime = new StandaloneOperatorRuntime({
      rootDir,
      // Deterministic spawn: return a fake pid without executing a real launch
      // script, so background-task state is governed solely by explicit
      // writeState calls below (no real subprocess racing the assertions).
