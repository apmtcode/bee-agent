...
describe("OperatorControlPlaneServer", () => {
  it("handles session, transcript, approval, trajectory, memory, and orchestration methods", async () => {
    ...
    const runtime = new StandaloneOperatorRuntime({
      ...
      // Inert launcher so long-running commands (e.g. `sleep 5`) don't leave a
      // real detached process — and don't race their initial state write against
      // the remote-status assertions below.
