...
describe("OperatorControlPlaneServer", () => {
  it("handles session, transcript, approval, trajectory, memory, and orchestration methods", async () => {
    ...
    const runtime = new StandaloneOperatorRuntime({
      rootDir,
      // Inert spawn: this test drives execution state manually, so a real
      // launcher subprocess would race the recovery assertions (see the
      // matching note in operator-runtime.test.ts).
    ...
    const driftingRuntime = new StandaloneOperatorRuntime({
      ...
      backgroundTaskSpawnProcess: () => ({ pid: 100003, unref() {} }),
      backgroundTaskIsProcessRunning: () => false,
    });
    ...
    const breakerRuntime = new StandaloneOperatorRuntime({
      ...
      backgroundTaskSpawnProcess: () => ({ pid: 100002, unref() {} }),
      backgroundTaskIsProcessRunning: () => false,
    });
