...
describe("OperatorControlPlaneServer", () => {
  it("handles session, transcript, approval, trajectory, memory, and orchestration methods", async () => {
    ...
    const runtime = new StandaloneOperatorRuntime({
      rootDir,
      // Mock the spawn so background-task launch scripts do not run as real
      // detached processes. Their asynchronous "running" state writes otherwise
      // race the control-plane's remote-status diagnostics (which combine
      // isProcessRunning() with the persisted state) non-deterministically.
