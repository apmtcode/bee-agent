...
describe("OperatorControlPlaneServer", () => {
  it("handles session, transcript, approval, trajectory, memory, and orchestration methods", async () => {
    ...
    const runtime = new StandaloneOperatorRuntime({
      rootDir,
      // Mock the spawn so background tasks don't actually run their bash launch
      // script: otherwise the script writes a "running" state file that races
      // with these assertions (with isProcessRunning=false, a written running
      // state flips remote diagnostics to "missing-process"/degraded). The
      // launch script itself is covered by background-tasks.test.ts.
