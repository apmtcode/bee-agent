...
// Never spawn a real launch script in unit tests: background-task execution
// state is driven deterministically via the store, so a real detached process
// would race those transitions (and leak). Each call returns a distinct fake pid.
let fakeSpawnCounter = 0;
const noopBackgroundSpawn = () => ({ pid: 40_000 + ++fakeSpawnCounter, unref() {} });
