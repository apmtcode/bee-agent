...
describe("OperatorControlPlaneServer", () => {
  it("handles session, transcript, approval, trajectory, memory, and orchestration methods", async () => {
    ...
    const runtime = new StandaloneOperatorRuntime({
      rootDir,
      // Deterministic launcher: the stub writes no execution state file, so a
      // freshly started task has no persisted state (reads as "active" until an
      // explicit state is written, and resolves to NOT_FOUND / missing-process
      // on sync) instead of racing a real detached subprocess.
