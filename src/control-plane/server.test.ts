...
describe("OperatorControlPlaneServer", () => {
  it("handles session, transcript, approval, trajectory, memory, and orchestration methods", async () => {
    ...
    const runtime = new StandaloneOperatorRuntime({
      rootDir,
      // Deterministic launch: never spawn a real OS process, so the test's
      // execution-state assertions aren't raced by a real background process
      // writing its own state files.
