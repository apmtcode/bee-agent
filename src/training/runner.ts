...
function renderLaunchScript(execution: LocalTrainingExecution, plan: TrainingJobPlan): string {
  ...
  return [
    ...
    // Backslashes must reach the emitted script (`s/\"\$\$\"/$$/g`) so bash keeps
    // the quotes/`$$` literal for sed; a single-escaped literal emits
    // `s/"$$"/$$/g`, whose bare `"` breaks bash quoting and leaves `"pid":"$$"`.
