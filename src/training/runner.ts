...
function renderLaunchScript(execution: LocalTrainingExecution, plan: TrainingJobPlan): string {
  ...
  return [
    ...
    // Replace the quoted PID placeholder with the bare shell PID ($$) so `pid`
    // is written as a JSON number; quoting the placeholder in the pattern keeps
    // $$ out of any sed pattern (where it would self-match after expansion).
