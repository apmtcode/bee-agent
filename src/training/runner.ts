...
function renderLaunchScript(execution: LocalTrainingExecution, plan: TrainingJobPlan): string {
  ...
  return [
    ...
    // Substitute placeholders (including the literal `"$$"` pid placeholder → numeric
    // shell pid) and rename into place so readers never observe a torn write. The
    // doubled backslashes make sed receive a literal `"$$"` search pattern.
