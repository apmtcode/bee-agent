...
function renderLaunchScript(execution: LocalTrainingExecution, plan: TrainingJobPlan): string {
  ...
  // The initial "running" state is emitted by python3 from a base64-encoded JSON
  // payload instead of a printf|sed pipeline, so command arguments containing
  // quotes or newlines cannot corrupt the state.json that readState later parses.
  const initialStateBase64 = Buffer.from(
    JSON.stringify({
      version: 1,
      jobId: plan.jobId,
      status: "running",
