...
function renderLaunchScript(execution: LocalTrainingExecution, plan: TrainingJobPlan): string {
  ...
  // timestamps at runtime via python so the state JSON is always well-formed.
  // A `printf | sed` bootstrap corrupts the JSON when the command array contains
  // quotes and never substitutes the pid placeholder cleanly.
