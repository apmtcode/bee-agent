...
function renderLaunchScript(execution: LocalTrainingExecution, plan: TrainingJobPlan): string {
  ...
  return [
    ...
    // Write the base payload verbatim then let python inject the real pid ($$) and
    // timestamp. The previous `sed "s/\"$$\"/$$/g"` never substituted because `$$`
    // expands inside the double-quoted sed script (pattern became "<pid>").
