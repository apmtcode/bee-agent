...
function renderLaunchScript(execution: LocalTrainingExecution, plan: TrainingJobPlan): string {
  ...
  return [
    ...
    // Atomic write (temp + mv) so a concurrent state reader never sees a
    // truncated JSON document mid-write.
...
function renderStateWriterPython(status: TrainingExecutionState["status"]): string[] {
  return [
    ...
    // Atomic replace so a concurrent state reader never sees a partial write.
    "import os",
