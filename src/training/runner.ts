...
function renderLaunchScript(execution: LocalTrainingExecution, plan: TrainingJobPlan): string {
  ...
  // Emit the initial "running" state via Python from a base64-encoded payload
  // rather than fragile `printf | sed` munging (which corrupts command/path
  // fields containing quotes or `$` and never reliably substitutes the pid).
