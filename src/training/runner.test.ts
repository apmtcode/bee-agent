...
describe("LocalAppleSiliconTrainingRunner", () => {
  it("builds and persists an SFT runner plan", async () => {
    ...
    await expect(runner.readLaunchScript({ ...job, execution })).resolves.toEqual(
      // State is written atomically: redirect to a temp file, then rename into place.
      expect.stringContaining(`> '${execution.stateFile}.tmp' && mv -f '${execution.stateFile}.tmp' '${execution.stateFile}'`),
    );
