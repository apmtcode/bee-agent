...
describe("StandaloneOperatorRuntime", () => {
  ...
  it("starts, syncs, recovers, lists, and cancels background tasks", async () => {
    const runtime = new StandaloneOperatorRuntime({
      rootDir: await makeTempDir(),
      // Inject a no-op launcher so the test drives execution state deterministically
      // via writeState/writeOutput instead of racing a real detached background
      // process that concurrently rewrites the same state file.
