...
describe("StandaloneOperatorRuntime", () => {
  ...
  it("starts, syncs, recovers, lists, and cancels background tasks", async () => {
    const runtime = new StandaloneOperatorRuntime({
      rootDir: await makeTempDir(),
      // Deterministic launcher: never spawn a real detached process, so the
      // test's explicit writeState() calls fully control execution state and
      // never race an OS subprocess writing the same state file.
