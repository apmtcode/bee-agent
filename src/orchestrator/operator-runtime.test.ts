...
describe("StandaloneOperatorRuntime", () => {
  ...
  it("starts, syncs, recovers, lists, and cancels background tasks", async () => {
    const runtime = new StandaloneOperatorRuntime({
      rootDir: await makeTempDir(),
      // Drive execution state via explicit writeState/writeOutput below; the
      // no-op spawn keeps the real detached process from racing in (and avoids
      // leaking a real `tail -f` process from the monitor task).
