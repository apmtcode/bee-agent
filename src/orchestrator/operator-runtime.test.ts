...
describe("StandaloneOperatorRuntime", () => {
  ...
  it("starts, syncs, recovers, lists, and cancels background tasks", async () => {
    const runtime = new StandaloneOperatorRuntime({
      rootDir: await makeTempDir(),
      // Stub the launcher so the test's own writeState calls are authoritative
      // (a real subprocess would asynchronously overwrite them and race).
      backgroundTaskSpawnProcess: createDeterministicBackgroundSpawn().spawnProcess,
